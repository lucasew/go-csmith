...
// Callers must pass an explicit *Session to *Sess Rng methods (or use rSess(r)).
...
// NewRngSess is NewRng bound to an explicit residual sticky bag.
func NewRngSess(s *Session, seed uint64) *Rng {
	r := NewRng(seed)
	r.Sess = s
	return r
}
