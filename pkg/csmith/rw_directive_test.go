...
func TestStepHashEmittedInBlock(t *testing.T) {
	...
	b := MakeRandomBlock(NewRngSess(testAmbientSession, 3), opts, probs, vs, NewExprTablesSess(testAmbientSession, opts), NewStatementThresholdTableSess(testAmbientSession, opts), &cg, false)
	if b == nil {
		t.Fatal("nil block")
	}
	if !b.EmitStepHash {
