...
func VisitFactsStatementReturn(st *Stmt, cg *CGContext, opts Options) bool {
	...
	if opts.NoReturnDeadPointer && st.Expr.Term == TermVariable && st.Expr.Var != nil {
		...
		ind, iok := st.Expr.IndirectLevelCompleteSess(sessFromCG(cg))
		if !iok {
			noteErrCG(cg, ErrGeneric)
			return false
		}
		facts := cg.pointToFacts()
		if IsPointingToLocalsSess(sessFromCG(cg), v, b, ind, facts) {
...
func MakeRandomReturn(
	...
	if DepthGuardByTypeSess(sessFromCG(cg), opts, DtStatementReturn) == BadDepth {
		return Stmt{}
	}
