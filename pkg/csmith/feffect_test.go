...
func TestNoteWriteDoesNotTouchFEffect(t *testing.T) {
	...
	l := CreateVariableQferSess(testAmbientSession, "l_1", GetIntTypeSess(testAmbientSession), NewCVQualifiers([]bool{false}, []bool{false}))
	cg.NoteWrite(g)
	cg.NoteWrite(l)
...
func TestCommentOutputInsertionOrderAndFormat(t *testing.T) {
	...
	a := CreateVariableQferSess(testAmbientSession, "g_a", GetIntTypeSess(testAmbientSession), NewCVQualifiers([]bool{false}, []bool{false}))
	b := CreateVariableQferSess(testAmbientSession, "g_b", GetIntTypeSess(testAmbientSession), NewCVQualifiers([]bool{false}, []bool{false}))
	c := CreateVariableQferSess(testAmbientSession, "g_c", GetIntTypeSess(testAmbientSession), NewCVQualifiers([]bool{false}, []bool{false}))
