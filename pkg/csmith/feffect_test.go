...
func TestNoteWriteDoesNotTouchFEffect(t *testing.T) {
	...
	l := CreateVariableQferSess(testAmbientSession, "l_1", GetIntTypeSess(testAmbientSession), NewCVQualifiersSess(testAmbientSession, []bool{false}, []bool{false}))
	cg.NoteWrite(g)
	cg.NoteWrite(l)
...
func TestFunctionOutputFEffectComment(t *testing.T) {
	f := &Function{Name: "func_1", ReturnType: GetIntTypeSess(testAmbientSession), EmitConcise: false}
	g := CreateVariableQferSess(testAmbientSession, "g_9", GetIntTypeSess(testAmbientSession), NewCVQualifiersSess(testAmbientSession, []bool{false}, []bool{false}))
...
func TestCommentOutputInsertionOrderAndFormat(t *testing.T) {
	...
	a := CreateVariableQferSess(testAmbientSession, "g_a", GetIntTypeSess(testAmbientSession), NewCVQualifiersSess(testAmbientSession, []bool{false}, []bool{false}))
	b := CreateVariableQferSess(testAmbientSession, "g_b", GetIntTypeSess(testAmbientSession), NewCVQualifiersSess(testAmbientSession, []bool{false}, []bool{false}))
	c := CreateVariableQferSess(testAmbientSession, "g_c", GetIntTypeSess(testAmbientSession), NewCVQualifiersSess(testAmbientSession, []bool{false}, []bool{false}))
