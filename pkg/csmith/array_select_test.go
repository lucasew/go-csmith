...
func TestSelectArrayChoosesExisting(t *testing.T) {
	...
	got := vs.SelectArray(NewRngSess(testAmbientSession, 3), EmptyCGContext().WithSession(testAmbientSession))
	if got != a && got != b {
		t.Fatal(got)
	}
}
...
func TestMakeRandomArrayInitMultiDimNested(t *testing.T) {
	...
	st := MakeRandomArrayInit(NewRngSess(testAmbientSession, 9), opts, probs, vs, tables, stmtTab, &cg)
...
func TestSelectArrayNilHoleFailClosed(t *testing.T) {
	...
	if vs.SelectArray(NewRngSess(testAmbientSession, 1), EmptyCGContext().WithSession(testAmbientSession)) != nil {
	...
	if vs.SelectArray(NewRngSess(testAmbientSession, 4), cg2) != nil {
