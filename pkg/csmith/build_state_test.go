...
func TestBuildStateTransitions(t *testing.T) {
	...
	f.GenerateBody(NewRngSess(testAmbientSession, 2), opts, probs, vs, NewExprTablesSess(testAmbientSession, opts), NewStatementThresholdTableSess(testAmbientSession, opts), WithFunc(f, EmptyEffect()).WithSession(testAmbientSession))
	if f.BuildState != BuildBuilt || !f.IsBuilt || !f.IsEffectKnownSess(testAmbientSession) {
		t.Fatalf("built %v", f.BuildState)
	}
	// regenerate ignored
	body := f.Body
	f.GenerateBody(NewRngSess(testAmbientSession, 3), opts, probs, vs, NewExprTablesSess(testAmbientSession, opts), NewStatementThresholdTableSess(testAmbientSession, opts), WithFunc(f, EmptyEffect()).WithSession(testAmbientSession))
	if f.Body != body {
		t.Fatal("regen")
	}
}
...
func TestPointerParamTBD(t *testing.T) {
	...
	f.GenerateBody(NewRngSess(testAmbientSession, 2), opts, probs, vs, NewExprTablesSess(testAmbientSession, opts), NewStatementThresholdTableSess(testAmbientSession, opts), cg)
...
func TestMakeFirstMarksBuilt(t *testing.T) {
	...
	f := MakeFirst(NewRngSess(testAmbientSession, 2), opts, probs, vs, &vs.Sym, NewExprTablesSess(testAmbientSession, opts), NewStatementThresholdTableSess(testAmbientSession, opts), nil, nil)
...
func TestGenerateBodyIncompleteAmbientResidualSticky(t *testing.T) {
	...
	f.GenerateBody(NewRngSess(testAmbientSession, 1), opts, NewProbabilities(opts), NewVariableSelector(testAmbientSession, opts), NewExprTablesSess(testAmbientSession, opts), NewStatementThresholdTableSess(testAmbientSession, opts), prev)
