...
func TestNeedNestedLoopNilRWHoleFailClosed(t *testing.T) {
	...
	if !b.NeedNestedLoop(cg2, NewRngSess(testAmbientSession, 1)) {
...
func TestEffectCloneIndependent(t *testing.T) {
	...
	e = e.ReadVarSess(testAmbientSession, CreateVariableScalarsSess(testAmbientSession, "g_2", GetIntType(), false, false))
	if c.IsReadSess(testAmbientSession, CreateVariableScalarsSess(testAmbientSession, "g_2", GetIntType(), false, false)) {
	...
	_ = e2.WriteVarSess(testAmbientSession, CreateVariableScalarsSess(testAmbientSession, "g_3", GetIntType(), false, false))
...
func TestInArrayLoopFromIVBounds(t *testing.T) {
	...
	b := MakeRandomBlock(NewRngSess(testAmbientSession, 1), opts, NewProbabilities(opts), NewVariableSelector(testAmbientSession, opts), NewExprTables(opts), NewStatementThresholdTable(opts), &cg, false)
...
func TestAppendReturnStmtRecordsMaps(t *testing.T) {
	...
	if st2 := b.AppendReturnStmt(nil, opts, NewVariableSelector(testAmbientSession, opts), &cg); st2 != nil {
	...
	f.Stack = []*Block{bad}
	if bad.AppendReturnStmt(NewRngSess(testAmbientSession, 2), opts, NewVariableSelector(testAmbientSession, opts), &cg) != nil {
...
func TestAddNewVarFactIntoNilFieldHoleFailClosed(t *testing.T) {
	...
	q := CreateVariableScalarsSess(testAmbientSession, "g_s.q", PointerTo(GetIntType()), false, false)
	if q == nil {
		t.Fatal("live pointer field var")
	}
	v.FieldVars = []*Variable{nil, q}
	// seed a prior fact so clear is observable vs empty start
	prior := MakeFactPointTo(CreateVariableScalarsSess(testAmbientSession, "g_other", PointerTo(GetIntType()), false, false), NullPtr)
	...
	AddNewVarFactInto(CreateVariableScalarsSess(testAmbientSession, "g_n", GetIntType(), false, false), nil)
	if !HasErrorSess(testAmbientSession) {
...
func TestDropUnionLocalsSyncsCurrentUnionsForSameFacts(t *testing.T) {
	...
	loc := CreateVariableScalarsSess(testAmbientSession, "l_local_u", ut, false, false)
...
func TestPostCreationFPOnlyOnHasEdgeIn(t *testing.T) {
	...
	st := Stmt{Kind: StmtAssign, StmID: 51,
		LhsVar: CreateVariableScalarsSess(testAmbientSession, "g_x", GetIntType(), false, false),
	...
	fm.SetMapFactsIn(50, []*FactPointTo{MakeFactPointTo(p, CreateVariableScalarsSess(testAmbientSession, "g_t", GetIntType(), true, false))})
	cg := EmptyCGContext().WithSession(testAmbientSession).WithFactMgr(fm)
	eff := EmptyEffect()
	cg.EffectAccum = &eff
...
func TestGetDimension(t *testing.T) {
	...
	if sc.GetDimensionSess(testAmbientSession) != 0 {
		t.Fatal("scalar")
	}
	opts := Defaults()
	...
	av.Sizes = []int{2, 3, 4}
	if av.Variable.GetDimensionSess(testAmbientSession) != 3 {
		t.Fatal(av.Variable.GetDimensionSess(testAmbientSession))
	}
	// IsArray without AsArray soft invent was dim from ArraySizes; fair sticky 0
	ClearErrorSess(testAmbientSession)
	v := CreateVariableScalarsSess(testAmbientSession, "g_b", GetIntType(), false, false)
	v.IsArray = true
	v.ArraySizes = []int{5, 6}
	if v.GetDimensionSess(testAmbientSession) != 0 {
		t.Fatal("IsArray without AsArray GetDimension must fail closed 0, got", v.GetDimensionSess(testAmbientSession))
	}
	if !HasErrorSess(testAmbientSession) {
...
func TestNeedNestedLoopUsesGetDimension(t *testing.T) {
	opts := Defaults()
	av := CreateArrayVariable(NewRngSess(testAmbientSession, 2), opts, NewProbabilities(opts), nil, nil, nil, "g_m", GetIntType(), MakeInt(0), NewCVQualifiers([]bool{false}, []bool{false}))
	av.Sizes = []int{2, 3}
...
func TestAppendReturnStmtFiltersLocalOut(t *testing.T) {
	...
	eff := EmptyEffect()
	cg.EffectAccum = &eff
	st := body.AppendReturnStmt(NewRngSess(testAmbientSession, 3), opts, NewVariableSelector(testAmbientSession, opts), &cg)
	if st == nil || st.Kind != StmtReturn {
		t.Fatal(st)
	}
...
func TestAppendNestedLoopBumpsBlkDepthAroundFor(t *testing.T) {
	...
	pre := cg.BlkDepth
	_ = b.AppendNestedLoop(NewRngSess(testAmbientSession, 42), opts, NewProbabilities(opts), NewVariableSelector(testAmbientSession, opts), NewExprTables(opts), NewStatementThresholdTable(opts), &cg)
...
func TestAppendNestedLoopERRORGuard(t *testing.T) {
	...
	if b.AppendNestedLoop(NewRngSess(testAmbientSession, 1), opts, NewProbabilities(opts), NewVariableSelector(testAmbientSession, opts), NewExprTables(opts), NewStatementThresholdTable(opts), &cg) != nil {
		t.Fatal("sticky error must not append nested for")
	}
	ClearErrorSess(testAmbientSession)
	// Block.cpp always has RNG for make_random(eFor)
	if b.AppendNestedLoop(nil, opts, NewProbabilities(opts), NewVariableSelector(testAmbientSession, opts), NewExprTables(opts), NewStatementThresholdTable(opts), &cg) != nil {
	...
	if b.AppendNestedLoop(NewRngSess(testAmbientSession, 2), opts, NewProbabilities(opts), NewVariableSelector(testAmbientSession, opts), NewExprTables(opts), NewStatementThresholdTable(opts), &cgInc) != nil {
...
func TestMakeRandomAssignRejectsConstStruct(t *testing.T) {
	...
	got := MakeRandomAssign(NewRngSess(testAmbientSession, 1), opts, NewProbabilities(opts), NewVariableSelector(testAmbientSession, opts), NewExprTables(opts), &cg, st)
	if stmtOK(got) {
	...
	ClearErrorSess(testAmbientSession)
	got2 := MakeRandomAssign(NewRngSess(testAmbientSession, 1), opts, NewProbabilities(opts), NewVariableSelector(testAmbientSession, opts), NewExprTables(opts), &cg, hole)
	if stmtOK(got2) {
	...
	got3 := MakeRandomAssign(NewRngSess(testAmbientSession, 1), optsSV, NewProbabilities(optsSV), NewVariableSelector(testAmbientSession, optsSV), NewExprTables(optsSV), &cg, hole)
	if stmtOK(got3) {
...
func TestAppendReturnStmtVisitFailSetsError(t *testing.T) {
	...
	st := b.AppendReturnStmt(NewRngSess(testAmbientSession, 1), opts, vs, &cg)
...
func TestContainsBackEdgeIncompleteSticky(t *testing.T) {
	ClearErrorSess(testAmbientSession)
	if (*Block)(nil).ContainsBackEdge(NewFactMgrSess(testAmbientSession, nil)) {
...
func TestFindFixedPointAssignDerefFailsOnMayNull(t *testing.T) {
	...
	opts := Defaults()
	f := &Function{Name: "f", ReturnType: GetIntType()}
	pointee := CreateVariableScalarsSess(testAmbientSession, "g_x", GetIntType(), false, false)
...
func TestPostCreationDefersOOSUntilAfterFP(t *testing.T) {
	...
	bodyLoc := CreateVariableScalarsSess(testAmbientSession, "l_body", GetIntType(), false, false)
...
func TestPostCreationNoFPOOSsLiveUnionsNotMapOut(t *testing.T) {
	...
	param := CreateVariableScalarsSess(testAmbientSession, "p_1", PointerTo(GetIntType()), false, false)
	...
	for _, f := range outPT {
		if f != nil && f.Var != nil && (f.Var == param || f.Var.MatchSess(testAmbientSession, param)) {
