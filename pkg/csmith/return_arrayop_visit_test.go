...
func TestVisitFactsStatementArrayOpInit(t *testing.T) {
	...
	st := MakeRandomArrayInit(NewRngSess(testAmbientSession, 2), opts, NewProbabilities(opts), vs, NewExprTablesSess(testAmbientSession, opts), NewStatementThresholdTableSess(testAmbientSession, opts), &cg)
