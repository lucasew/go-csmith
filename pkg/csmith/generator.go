...
// postAggPostCD3ArrayOp2GlobalU56Done: one-shot e9274 GlobalList U56; later U2.
var postAggPostCD3ArrayOp2GlobalU56Done bool
// postAggPostCD3ArrayOp2GlobalU2Done: one-shot e9740 U2 fail→PL; later Global.
var postAggPostCD3ArrayOp2GlobalU2Done bool
// postAggPostCD3ArrayOp2GlobalSoleN: free Expression Global after U2 era
// (0: e9908 sole; 1+: e9922 U61 + itemize U2 U9 U4).
var postAggPostCD3ArrayOp2GlobalSoleN int
...
type functionFlowState struct {
	...
	// postAggPostCD3ArrayOp2PLSoleUsed: after first sole, later PL U5 choose (e9834).
	...
	postAggPostCD3ArrayOp2FuncArgMustDone bool
	// postAggPostCD3ArrayOp2FuncArgPPCreateDone: e9945 PP→PL create after must miss.
	postAggPostCD3ArrayOp2FuncArgPPCreateDone bool
	...
	// postAggPostCD3ArrayOp2EmptyCreateN: empty SelectDeref create count
	// (0: e9463 U7; 1: e9616 U2; 2+: e9672 nested NewArray+CreateArray).
	postAggPostCD3ArrayOp2EmptyCreateN int
	// postAggPostCD3ArrayOp2NeedNoRhsN: need_no_rhs Lhs must_use WRITE count
	// (0: e9222 U5 U5 U3 U3 F75; 1: e9651 U4 U3 U4 U3 U2 F75 F80=0→VS).
	postAggPostCD3ArrayOp2NeedNoRhsN int
	// postAggPostCD3ArrayOp2PLQferN: ArrayOp2 PL create qfer pad count
	// (0: e9468 **** floor3; 1+: natural levels e9719 **).
	postAggPostCD3ArrayOp2PLQferN int
...
func trySelectMustUseVar(er *exprRand, t CType, ctx *genContext) (exprVarCandidate, bool) {
	...
	// e9940–: ArrayOp2 Function-arg must_use U5 U7 F75 F0 → VS PP U100
	// PL U6 + F50 F10 (1-level qfer) F20 NewArray=0 + struct init residual.
	// After GlobalSoleN≥2 (e9908 sole + e9922 U61 itemize era).
	if arrayOp2Must && postAggPostCD3ArrayOp2GlobalSoleN >= 2 && er.fallback != nil &&
		!st.postAggPostCD3ArrayOp2FuncArgMustDone {
		st.postAggPostCD3ArrayOp2FuncArgMustDone = true
		st.postAggPostCD3ArrayOp2FuncArgPPCreateDone = true
		_ = er.fallback.upto(5)       // e9940
		_ = er.fallback.upto(7)       // e9941
		_ = er.fallback.flipcoin(75)  // e9942
		_ = er.fallback.flipcoin(0)   // e9943
		_ = er.fallback.upto(100)    // e9944 VS U100 PP
		_ = parentStackPick(er, st)  // e9945 U6
		st.postAggPostCD3ArrayOp2PLSole = false
		// e9946–47: one ptr-level + self qfer is only F50 F10 (not ****×4).
		_ = er.fallback.flipcoin(50)
		_ = er.fallback.flipcoin(10)
		_ = er.fallback.flipcoin(20) // e9948 NewArray=0
		// e9949–: make_init Constant path for struct S0 (bitfields U181 + fields).
		// Use real aggregate formatter so hex pure_rnd digits stay LCG-aligned.
		opts := Options{EnableFloat: false, Int128: false, UInt128: false}
		if ctx != nil {
			// Prefer live opts if available via state — none stored; defaults ok.
		}
		s0 := CType{Name: "struct S0", Signed: true, Bits: 32, HexDigits: 8}
		_ = formatAggregateOrSimpleConstant(er.fallback, s0, ctx, opts)
		// e9972–88: create_field_vars Constant stream after aggregate under-count.
		burnCreateFieldVarsConstants(er.fallback, s0, ctx, opts)
		return exprVarCandidate{expr: "x", ctype: t, assignable: true}, true
	}
...
func randomConstantExprFromER(t CType, er *exprRand, opts Options) string {
	...
	if r.flipcoin(50) {
		...
		if smallDec {
			...
			// (e8410 U120=110).
			// e9913 ArrayOp2 late: after PP create, Constant U20 then Function
			// (do not arm depthBlock — would force Variable tries=7).
...
// burnLhsResidualScopeCreate mirrors residual-era VS create after ok_vars miss:
// qfer F50 (vol residual) + create_and_initialize (NewArray F20 + Constant +
// optional CreateArray U99…itemize). Used by long Lhs SelectDeref residual
// e13484–15095 (e14315 NewArray path U20 U99 U10×3 U48 + inits + itemize).
func burnLhsResidualScopeCreate(r *rng, opts Options) {
	if r == nil {
		return
	}
	_ = r.flipcoin(50) // residual qfer vol F50
	t := CType{Name: "int", Signed: true, Bits: 32, HexDigits: 8}
	_ = burnCreateAndInitialize(r, opts, t)
}
...
func createOnDemandFromParentLocalPathEROpts(er *exprRand, opts Options, t CType, ctx *genContext, qferMode int, retype bool, stackIndex int) (exprVarCandidate, bool) {
	...
	if qferMode > 0 {
		...
		// e9468–75: first ArrayOp2 PL create **** (3 levels + self = F50 F10×4).
		// e9719+: second ** (2 levels + self = F50 F10×3).
		// e10000: after Function-arg must residual, PL create is NewArray F20 F20
		// without random_qualifiers (skip floors; caller should pass qferMode 0).
		if postAggPostCD3ArrayOp2BodyActive && isPtr &&
			!ctx.state.postAggPostCD3ArrayOp2FuncArgMustDone {
			n := ctx.state.postAggPostCD3ArrayOp2PLQferN
			ctx.state.postAggPostCD3ArrayOp2PLQferN++
			switch {
			case n == 0 && levels < 3:
				levels = 3 // e9468 ****
			case n >= 1 && n < 3 && levels < 2:
				levels = 2 // e9719 / e9753 **
			case n >= 3:
				levels = 1 // e9821 * + self
			}
		}
		...
		if mustReadLiveSink != nil && !*mustReadLiveSink && postMustReadGlobalPicks != nil {
			*postMustReadGlobalPicks++
			...
			if multiDimArraySink != nil && *multiDimArraySink > 0 && n >= 2 {
				...
				// e9274: ArrayOp2 body free Expression GlobalList U56 once.
				// e9740+: later U2 choose then visit-fail → VS PL reselect.
				arrayOp2Body := postAggPostCD3ArrayOp2BodyActive ||
					(postAggPostCD3ArrayOp2BodySink != nil && *postAggPostCD3ArrayOp2BodySink)
				if arrayOp2Body {
					if !postAggPostCD3ArrayOp2GlobalU56Done {
						postAggPostCD3ArrayOp2GlobalU56Done = true
						target = 56
					} else if !postAggPostCD3ArrayOp2GlobalU2Done {
						// e9740–42: one-shot U2 then empty → VS PL reselect.
						postAggPostCD3ArrayOp2GlobalU2Done = true
						_ = er.pick(2)
						return exprVarCandidate{expr: "", ctype: t, assignable: false}, true
					}
					// e9908+: later Global sole (no sticky U2).
				}
				...
				// e9274: ArrayOp2 body — force U56 once; e9740 one-shot U2; later sole.
				arrayOp2GlobalU2Fail := false
				if postAggPostCD3ArrayOp2BodyActive {
					if !postAggPostCD3ArrayOp2GlobalU56Done {
						postAggPostCD3ArrayOp2GlobalU56Done = true
						target = 56
						...
						postAggPostCD3ArrayOp2GlobalU2Done = true
						target = 2
						// e9740–41: U2 choose then visit fail → VS PL reselect U100 U6.
						arrayOp2GlobalU2Fail = true
					}
				}
				...
				if target > 0 && target != n {
					...
					if arrayOp2GlobalU2Fail {
						// Burn U2 then miss so caller VariableSelector retries U100.
						_ = v
						return exprVarCandidate{}, false
					}
					...
					if len(exactNoArr) == 2 && postAggGlobalLivePicks == 3 {
						...
						if afterPPVisit {
							...
							if postPtrN >= 2 {
								...
								targetN = 44
								// e9274: ArrayOp2 body GlobalList U56 once; e9740 one-shot U2; later natural.
							if postAggPostCD3ArrayOp2BodyActive {
								if !postAggPostCD3ArrayOp2GlobalU56Done {
									postAggPostCD3ArrayOp2GlobalU56Done = true
									targetN = 56
	...
	for tries := 0; tries < 6; tries++ {
		...
		case termVariable:
			...
			if scopePick == 0 && flow != nil && flow.postAggPostCD3ArrayOp2Body &&
				...
				// e9740–48: one-shot after U56, Global U2→PL create.
				if postAggPostCD3ArrayOp2GlobalU56Done && !postAggPostCD3ArrayOp2GlobalU2Done {
					postAggPostCD3ArrayOp2GlobalU2Done = true
					_ = er.pick(2) // e9740
					scopePick2 := variableScopePickFromER(er, opts, &scope) // e9741
					if scopePick2 == 1 || scopePick2 == 4 {
						_ = parentStackPick(er, flow) // e9742 U6
						// Manual residual: avoid postCD3 NVPL mode2 F10-first.
						_ = er.fallback.flipcoin(50) // e9743 vol
						_ = er.fallback.flipcoin(10) // e9744 const
						_ = er.fallback.flipcoin(20) // e9745 NewArray=0
				...
				if postAggPostCD3ArrayOp2GlobalU2Done {
					...
					if n == 0 {
						// e9908: sole Global → next Expression U120 Constant.
						bumpExprDepth(ctx)
						markVarSelectEffect()
						return finishVar(castLiteral(t, "g_0"))
					}
			...
			if scopePick == 1 {
				...
				if useBlockLocal {
					...
					if flow != nil && flow.postAggPostCD3ArrayOp2Body &&
						flow.postAggPostCD3ArrayOp2NeedNoRhsN >= 2 &&
						!flow.postAggPostCD3ArrayOp2PLU5F0Done &&
						er != nil && er.fallback != nil {
						flow.postAggPostCD3ArrayOp2PLU5F0Done = true
						flow.postAggPostCD3ArrayOp2StmtLhsU9 = true
						flow.postAggPostCD3ArrayOp2PLSole = false
						_ = er.pick(5)              // e9834 locals
						_ = er.fallback.flipcoin(0) // e9835 visit fail
						scopePick2 := variableScopePickFromER(er, opts, &scope) // e9836
						if scopePick2 == 1 || scopePick2 == 4 {
							_ = parentStackPick(er, flow) // e9837 U6
							flow.postAggPostCD3ArrayOp2PLSole = false
							_ = er.pick(4) // e9838 choose among block locals
						}
						bumpExprDepth(ctx)
						markVarSelectEffect()
						return finishVar(castLiteral(t, "x"))
			...
			// e9888: ArrayOp2 after StmtLhsU9 era — PP miss → PL U6 **** create
			// (GO param inventory sole-accepts; UP falls through).
			// Do NOT fire on e9846 PP before Statement Lhs U9 (StmtLhsU9 still armed).
			// e9944: re-fire after Function-arg must_use miss (FuncArgMustDone, PPFallDone cleared).
			if scopePick == 2 && flow != nil && flow.postAggPostCD3ArrayOp2Body &&
				flow.postAggPostCD3ArrayOp2PLU5F0Done &&
				!flow.postAggPostCD3ArrayOp2StmtLhsU9 &&
				!flow.postAggPostCD3ArrayOp2PPFallDone {
				flow.postAggPostCD3ArrayOp2PPFallDone = true
				candidates = nil
			}
			...
			// e9888 ArrayOp2 PPFallDone: force empty → PL **** create (not sole).
			...
			if len(candidates) == 0 {
				...
				if scopePick == 2 {
					...
					if flow != nil && flow.postAggPostCD3ArrayOp2Body &&
						flow.postAggPostCD3ArrayOp2PPFallDone && er != nil && er.fallback != nil {
						// parentStackPick already set PLSole; clear so we create not sole.
						flow.postAggPostCD3ArrayOp2PLSole = false
						// Manual residual matches UP qfer **** + NewArray CreateArray.
						for i := 0; i < 4; i++ {
							_ = er.fallback.flipcoin(50)
							_ = er.fallback.flipcoin(10)
						}
						...
						// Nested address residual F50 F20 F20 + CreateArray U99…
						_ = er.fallback.flipcoin(50)
						_ = er.fallback.flipcoin(20)
						_ = er.fallback.flipcoin(20)
						base := CType{Name: "int32_t", Signed: true, Bits: 32, HexDigits: 8}
			...
			if len(candidates) > 0 {
				if c, ok := selectExprVariableFromER(t, er, candidates, false); ok {
					...
					if c.expr == "" && scopePick == 0 && flow != nil &&
						flow.postAggPostCD3ArrayOp2Body && postAggPostCD3ArrayOp2GlobalU56Done &&
						er != nil {
						// U2 already burned inside selectExprVariableFromER.
						scopePick = variableScopePickFromER(er, opts, &scope) // e9741 U100
						if scopePick == 1 || scopePick == 4 {
							idx := parentStackPick(er, flow) // e9742 U6
		...
		case termAssign:
			...
			// (not sticky e9222 U5×2 sole).
			if needNoRhsExpr && ctx != nil && ctx.state != nil &&
				ctx.state.postAggPostCD3ArrayOp2Body && er != nil && er.fallback != nil {
				n := ctx.state.postAggPostCD3ArrayOp2NeedNoRhsN
				ctx.state.postAggPostCD3ArrayOp2NeedNoRhsN++
				if n == 0 {
					_ = er.fallback.upto(5)
					_ = er.fallback.upto(5)
					_ = er.fallback.upto(3)
				...
				if n == 1 {
					// e9651–: U4 U3 U4 U3 U2 F75 F80=0 → VS NewValue U100 U14
					// NewArray F20 + Constant init F50 + CreateArray U99… then F80 chain.
					_ = er.fallback.upto(4)
					_ = er.fallback.upto(3)
					_ = er.fallback.upto(4)
					_ = er.fallback.upto(3)
					...
					_ = er.fallback.flipcoin(75)
					if !er.fallback.flipcoin(80) {
						_ = variableScopePickFromER(er, opts, &scope) // e9658 U100
						base := pickSimpleNonVoid(er.fallback, opts)  // e9659 U14
						_ = er.fallback.flipcoin(20)                  // e9660 NewArray
						_ = formatSimpleConstant(er.fallback, base)
						_arr := burnCreateArrayVariable(er.fallback, opts, base, true)
						emitOrphanArrayGlobal(ctx, base, _arr)
					}
					...
					if !er.fallback.flipcoin(80) {
						_ = variableScopePickFromER(er, opts, &scope) // e9777 U100
						_ = er.pick(6)                                 // e9778 U6
						_ = er.fallback.flipcoin(20)                   // e9779 NewArray=1
						base := CType{Name: "int32_t", Signed: true, Bits: 32, HexDigits: 8}
						_ = formatSimpleConstant(er.fallback, base) // e9780–82 F50 F50 U3
						_arr := burnCreateArrayVariable(er.fallback, opts, base, true)
						emitOrphanArrayGlobal(ctx, base, _arr)
					}
					lhsFromDeref = false
			...
			if er != nil && er.fallback != nil {
				for {
					...
					// e9672: third — nested F20 NewArray F50 CreateArray multi-dim.
					if postAggPostCD3ArrayOp2BodyActive && !newArray && !initConst &&
						ctx != nil && ctx.state != nil {
						n := ctx.state.postAggPostCD3ArrayOp2EmptyCreateN
						ctx.state.postAggPostCD3ArrayOp2EmptyCreateN++
						if n == 0 {
							_ = er.pick(7) // e9463
							lhsFromDeref = true
							break
							...
							// e9672–: nested pointee NewArray F20 + Constant F50
							// + CreateArray U99 U10×3 U36 + alts; SafeOpFlags F50 U4.
							base := CType{Name: "int8_t", Signed: true, Bits: 8, HexDigits: 2}
							_ = er.fallback.flipcoin(20) // nested NewArray
							_ = formatSimpleConstant(er.fallback, base)
							var padSaved *bool
							...
							_arr := burnCreateArrayVariable(er.fallback, opts, base, false)
							if padSaved != nil {
							...
							emitOrphanArrayGlobal(ctx, base, _arr)
							_ = er.fallback.flipcoin(50) // e9713 SafeOpFlags
							_ = er.fallback.upto(4)      // e9714
							return finishAssignExpr(fmt.Sprintf("(%s++)", "x"))
						} else if n == 3 {
							// e9787–99: F20 init null F0 fail; F80 F20×3 F50 F50 U3
							// F50 U4 SafeOpFlags → parent Expression U120.
							if initConst {
								_ = er.fallback.flipcoin(0) // e9790 null validate
								continue
							}
							_ = er.fallback.flipcoin(20) // third F20 residual
							base := CType{Name: "int32_t", Signed: true, Bits: 32, HexDigits: 8}
							_ = formatSimpleConstant(er.fallback, base) // F50 F50 U3
							_ = er.fallback.flipcoin(50)                // e9798 SafeOpFlags
							_ = er.fallback.upto(4)                     // e9799
							return finishAssignExpr(fmt.Sprintf("(%s++)", "x"))
						} else {
...
lhsDerefLoop:
	for !lhsFromDeref {
															...
															for fi := 0; fi < 80; fi++ {
																...
																if vv >= 100 {
																	...
																	if assignQfer {
																		...
																		postCreateConstHex8Once := false  // e17517 RandomHexDigits(8)
																		postCreateConstSmallOnce := false // e17542 F50 small residual
																		// postCreateConstSmallNoVarOnly: after e18322 ConstSmall,
																		// do not re-arm VarOnly (next free Function).
																		postCreateConstSmallNoVarOnly := false
																		// postCreateConstCreateArrayOnce: e18363 Constant
																		// after freePP CreateArray → F80 create residual.
																		postCreateConstCreateArrayOnce := false
																		// postCreateFuncOnlyOnce: after late Comma
																		// (e15868), next Expression Function-only
																		// (e15869 U120=49 tries=1).
																		postCreateFuncOnlyOnce := false
																		...
																		// postCreateLatePPF80: one-shot e15735
																		// PP sole + F80 SelectDeref.
																		postCreateLatePPF80 := true
																		// postCreateLatePPN: late PP count after
																		// assignN>=6 (e15735=1 … e15881 → VarOnly).
																		postCreateLatePPN := 0
																		// postCreateLateConstCreate: one-shot
																		// e15769 Constant → Global create residual.
																		postCreateLateConstCreate := true
																		// postCreateLateConstN: late Constant hits
																		// after assignN≥6 (e15769 create; e15901
																		// F50 hex; e15903 F50 hex16 F80 residual).
																		postCreateLateConstN := 0
																		// postCreateLateCommaN: late Comma count
																		// (e15737 first; e15867 → FuncOnly).
																		postCreateLateCommaN := 0
																		// postCreateLateCommaF80: one-shot e15940
																		// Comma Lhs SelectDeref F80 residual after
																		// Assign n≥8; later late Commas use U14.
																		postCreateLateCommaF80 := true
																		// postCreateMustUseResidual: after e15940
																		// Comma residual, next Variable is U4 U2× F75.
																		postCreateMustUseResidual := false
																		// postCreateAssignStarN: count of armed
																		// star-WRITE Assigns after late useEx=0
																		// Global (e16096 **=1; e16244 ****=2).
																		postCreateAssignStarN := 0
																		// postCreateAssignStarArmed: next Assign
																		// n≥8 uses star WRITE qfer residual.
																		postCreateAssignStarArmed := false
																		// postCreateLateGlobalN: Global hits after
																		// late Comma F80 era (e16088 U9; e16165 F0).
																		postCreateLateGlobalN := 0
																		// postCreateForceStdfunc: after late Constant
																		// F50 residual, next Function is stdfunc F5
																		// (not afterAsg bare; e16183–84).
																		postCreateForceStdfunc := false
																		// postCreatePostArrayVar: after e16416
																		// CreateArray itemize residual ends, next
																		// Expressions are Variable multiphase.
																		postCreatePostArrayVar := false
																		postCreatePostArrayVarN := 0
																		// postCreateLatePLN: PL hits after late
																		// Comma F80 era (e16456 U1 U5; e16460 F80).
																		postCreateLatePLN := 0
																		// postCreateLateFreePPN: free Variable PP
																		// after post-array era (e16450 bare;
																		// e16541+ U2).
																		postCreateLateFreePPN := 0
																		// postCreatePostArrayDone: true after
																		// post-array Variable multiphase ends.
																		postCreatePostArrayDone := false
																		// postCreatePostArrayGlobalN: Global hits
																		...
																		// postCreateLateFreePLN: free PL after asgLhsDone
																		// (e17385 U4+qfer; e17515+ U4 sole).
																		postCreateLateFreePLN := 0
																		// postCreateLateFreeGlobalN: free Global after asgN≥2
																		// (e17436 U140; e17547 U138+create residual).
																		postCreateLateFreeGlobalN := 0
																		// postCreateUseExF0NestOnce: useEx=0 → F0 nest
																		// Expressions (e17604) not VS U100.
																		postCreateUseExF0NestOnce := false
																		...
																		// postCreateLateFreeUseExN: useEx hits after
																		// late free *WRITE Assign.
																		postCreateLateFreeUseExN := 0
																		// postCreateLateFreeCreateMissVar: after late
																		// free CREATE F20 U14 (useExN≥3), nested param
																		// Function useEx=0 (max-funcs fail) → ExprVar.
																		postCreateLateFreeCreateMissVar := false
																		// postCreateLateFreeUseExF0Once: after ExprVar
																		...
																		// postCreateLateFreeConstN: free invent Constants
																		// after late free F0 (hex size multiphase).
																		postCreateLateFreeConstN := 0
																		...
																		// postCreateNoConstOnce: e16496 no_const
																		// Expression (reject Constant only).
																		postCreateNoConstOnce := false
																		// postCreateAssignThenConst: after no_const
																		// Assign F50, next Expression Constant-only.
																		postCreateAssignThenConst := false
																		...
																		for j := 0; j < 500; j++ {
																			...
																			} else if postCreateNoConstOnce {
																				// e16496: reject Constant 90–99 only.
																				postCreateNoConstOnce = false
																				vv = rf.uptoWithFilter(120, func(x uint32) bool {
																					return x >= 90 && x < 100
																				})
																			...
																			} else if postCreateFuncOnlyOnce {
																				postCreateFuncOnlyOnce = false
																				// Function 0–69 only
																				vv = rf.uptoWithFilter(120, func(x uint32) bool {
																					return x >= 70
																			...
																			if vv >= 110 {
																				...
																				// is Lhs SelectDeref F80 ladder once.
																				// e15737+: after that Lhs residual ends,
																				// Comma again uses U14 (not re-enter).
																				if postCreateEra && postCreateUseExHitN >= 4 && !lhsSelectDerefResidualRan && !lhsResidualDone && postCreateAssignN < 6 {
																					// force Lhs path (e15483 era only)
																					lhsSelectDerefResidualRan = false
																				}
																				if (!postCreateEra || (postCreateUseExHitN >= 4 && postCreateAssignN < 6 && !lhsResidualDone)) && !depthFilterExpr && depthFilterClosed && depthConstSelectDerefDone && !lhsSelectDerefResidualRan {
																							...
																							if sp < 35 {
																								...
																								if postCreateUseExHitN >= 4 {
																									...
																									case 5:
																										...
																										_ = rf.upto(8)
																									default:
																										// e15708+: may Statement residual
																										// or smaller choose — U8 sole default
																										_ = rf.upto(8)
																									}
																								} else if lhsDerefN < 5 {
																									_ = rf.upto(5)
																									...
																									case globalVS == 14:
																										// e14644: U4 sole
																										_ = rf.upto(4)
																									case globalVS == 15, globalVS == 16, globalVS == 17, globalVS == 22:
																										// e14696/e14735/e14744/e14907: U3 U8
																										_ = rf.upto(3)
																										_ = rf.upto(8)
																									case globalVS == 18, globalVS == 20, globalVS == 21, globalVS == 23:
																										// e14763/e14820/e14878/e15044: U3 U10
																										_ = rf.upto(3)
																										_ = rf.upto(10)
																									case globalVS == 19:
																										// e14795: U3 U4
																								...
																								plVS++
																								if postCreateUseExHitN >= 4 {
																									// e15590: U5 U5; e15642: U5 U3;
																									// e15700: U5 U14 F50 F20 create.
																									_ = rf.upto(5)
																									switch {
																									case plVS == 1:
																										_ = rf.upto(5)
																									...
																									default:
																										// e15701–49: PL create
																										// U14 F50 F20 F50 F50 U4
																										// then VS Global U72 For
																										// residual → NewValue U120.
																										_ = rf.upto(14)
																										_ = rf.flipcoin(50)
																										_ = rf.flipcoin(20)
																										_ = rf.flipcoin(50)
																										// unlogged gap (~2 next31)
																										_ = rf.next31()
																										_ = rf.next31()
																										_ = rf.flipcoin(50)
																										_ = rf.upto(4)
																										// e15708–49: post-create
																										// VS/For residual (no F80).
																										_ = rf.upto(100) // Global
																										_ = rf.upto(72)
																										_ = rf.upto(2)
																										_ = rf.flipcoin(0)
																										...
																										// e15750: next U120 outside
																										// Lhs residual — break F80 loop.
																										// e15725+: post-Lhs Functions
																										// are stdfunc F5 (not useEx).
																										lhsResidualDone = true
																										postCreateUseExAgain = false
																									}
																								} else {
																								_ = rf.upto(5)
																								switch {
																								case plVS <= 2:
																									// e13502/e13525: U5 U3 U9
																									_ = rf.upto(3)
																									_ = rf.upto(9)
																								case plVS <= 4:
																									// e13540/e13602: itemize
																									_ = rf.upto(9)
																									_ = rf.upto(9)
																									_ = rf.upto(3)
																									_ = rf.flipcoin(0)
																								case plVS == 5:
																									// e13804: U5 sole
																								case plVS == 12, plVS == 17:
																									// e14187/e14669: itemize
																									_ = rf.upto(9)
																									_ = rf.upto(9)
																									_ = rf.upto(3)
																									_ = rf.flipcoin(0)
																								...
																								case plVS == 15:
																									...
																									_ = rf.upto(2)
																									_ = rf.upto(9)
																								case plVS == 19, plVS == 20:
																									// e14804/e14954: U3 U4 U8
																									_ = rf.upto(3)
																									_ = rf.upto(4)
																									_ = rf.upto(8)
																								case plVS == 21:
																								...
																								case ppVS == 4:
																									// e13840: U5 sole
																								case ppVS == 5:
																									// e13888: U5 F50 F20 Const create
																									burnLhsResidualScopeCreate(rf, opts)
																								case ppVS == 6, ppVS == 9, ppVS == 14:
																									// e14021/e14556/e14937:
																									...
																									_ = rf.flipcoin(0)
																								case ppVS == 7, ppVS == 8:
																									// e14315/e14434: create
																									// (+CreateArray when NewArray)
																									burnLhsResidualScopeCreate(rf, opts)
																								case ppVS == 10, ppVS == 13:
																									// e14633/e14931: U3 U4 U8
																									_ = rf.upto(3)
																									_ = rf.upto(4)
																									_ = rf.upto(8)
																								case ppVS == 11:
																								...
																								// NewValue: F10 scope
																								// (Global vs PL).
																								// e13816 N1: F10=0 U5 U14
																								// F50 F20 F50 hex → fail
																								// validate, more F80.
																								// e15083 N2: F10=0 U5 U14
																								// F50 F20 Const U3 → Lhs
																								// accept; residual ends.
																								nvVS++
																								if !rf.flipcoin(10) {
																									_ = rf.upto(5)
																									u14 := rf.upto(14)
																					...
																					// e15093–95: after Lhs residual
																					// accepts (NewValue N2):
																					// SafeOpFlags F50 + size U4,
																					// then Statement/VS U100 before
																					// next Expression U120.
																					// e15720+: post-CREATE PL-create end
																					// already burned NewValue U100;
																					// next is outer U120 (no trail).
																					if lhsResidualDone && postCreateUseExHitN < 4 {
																						_ = rf.flipcoin(50)
																						_ = rf.upto(4)
																						_ = rf.upto(100)
																					}
																				...
																				// e13009 Comma type=nil: filtered
																				...
																				if lhsSelectDerefResidualRan {
																					...
																					// e15738/e15868: NonVoid filter tries=1.
																					if postCreateAssignN >= 6 {
																						postCreateLateCommaN++
																						// e15940: first late Comma after
																						// Assign n≥8 → Lhs SelectDeref F80
																						// residual. e16085+: later late
																						// Commas → NonVoid U14 (same as
																						// assignN 6–7 path).
																						if postCreateAssignN >= 8 && postCreateLateCommaF80 {
																							postCreateLateCommaF80 = false
																							// e15941–68: F80 U3 U7 ×2 F80=0
																							// → PL itemize + F80 ladder →
																							// Global create residual.
																							for ri := 0; ri < 4; ri++ {
																								if !rf.flipcoin(80) {
																									_ = rf.upto(100) // PL
																									_ = rf.upto(6)
																									_ = rf.upto(5)
																									// e15951–54: itemize U9 U9 U3 F0
																									_ = rf.upto(9)
																									_ = rf.upto(9)
																									_ = rf.upto(3)
																									_ = rf.flipcoin(0)
																									// e15955–60: F80 U3 F80 U2 F0 F80=0
																									if rf.flipcoin(80) {
																										_ = rf.upto(3)
																									}
																									if rf.flipcoin(80) {
																									...
																									if !rf.flipcoin(80) {
																										// e15961–64: Global U4 F50 U4
																										// then outer U120 Variable.
																										_ = rf.upto(100)
																										_ = rf.upto(4)
																										_ = rf.flipcoin(50)
																										_ = rf.upto(4)
																										// e15965+: next Variable with
																										// U4 U2 multiphase residual.
																										postCreateMustUseResidual = true
																									}
																									break
																								}
																								_ = rf.upto(3)
																							...
																							// assignN≥8 late Comma U14
																							// does not re-arm FuncOnly
																							// (next is free Variable).
																							if postCreateAssignN < 8 && postCreateLateCommaN >= 4 {
																								postCreateFuncOnlyOnce = true
																							}
																							// e16555–58: first post-array
																							// late Comma → Function useEx.
																							// e16751+: later Commas no re-arm.
																							if postCreatePostArrayDone && !postCreateLateCommaUseExDone {
																			...
																			if vv >= 100 {
																				...
																				if postCreateAsgSelDerefOnce {
																					// e18475–542: Assign Lhs SelectDeref F80
																					// ladder after freePL n=11 residual.
																					postCreateAsgSelDerefOnce = false
																					// Head: F80 U6 U9 F0; F80 U6 F0; F80 U5;
																					// F80 U4; F80 U3 U9 F0; F80=0 → VS create.
																					if rf.flipcoin(80) {
																						_ = rf.upto(6)
																						_ = rf.upto(9)
																						_ = rf.flipcoin(0)
																					...
																					// Post-head F80 ladder: body reset after each VS.
																					// e18508+: U3; U2 U9 F0; F80=0 VS;
																					// then U2 U9 F0 / U2 / U9 F0 cycles.
																					vsN, bodyN, ppVS := 0, 0, 0
																					for ri := 0; ri < 80; ri++ {
																						if !rf.flipcoin(80) {
																							vsN++
																							bodyN = 0
																							...
																							if sp >= 65 && sp < 95 {
																								...
																								if ppVS == 1 {
																									...
																									// e18635: PP U6 U14 create + field Constant catalog
																									_ = rf.upto(6)
																									_ = rf.upto(14)
																									_ = rf.flipcoin(50)
																									_ = rf.flipcoin(20)
																									...
																									_ = rf.upto(10)
																									_ = rf.upto(10)
																									// e18646: nested Expression Variable invent then
																									// field Constant catalog F50×… (hex×16 on F50=0).
																									_ = rf.upto(120) // Variable 78
																									// field catalog e18647–802 (hex×16 on F50=0)
																									for fi := 0; fi < 78; fi++ {
																										burnFC(16)
																									}
																									// e18803–808: trailing U6 U8 U5 F50 U4 U100
																									_ = rf.upto(6)
																									_ = rf.upto(8)
																									_ = rf.upto(5)
																									_ = rf.flipcoin(50)
																									...
																									_ = rf.uptoWithFilter(100, func(x uint32) bool {
																										return x < 65 || x >= 95 // PP only tries=2
																									})
																									// e18809: next Function useExisting F50
																									postCreateForceStdfunc = false
																									postCreateUseExAgain = true
																									break
																								} else {
																								...
																								// e18543: Global U9 U4; e18585+: Global U9 U8
																								_ = rf.upto(9)
																								if vsN <= 3 {
																									_ = rf.upto(4)
																								} else {
																								...
																								// keep F80 ladder after Global residual
																							} else if sp < 65 {
																								// e18576: PL U6 U9 F0
																								_ = rf.upto(6)
																								_ = rf.upto(9)
																						...
																						bodyN++
																						// F80=1 body after vsN VS exits.
																						// vsN=1 create: U3 then U2 U9 F0.
																						// vsN=2 PP U6 U2: U2 U9 F0; U2; U9 F0×N.
																						// vsN>=3 Global+: U9 F0 sole.
																						switch {
																						case vsN <= 1:
																							if bodyN == 1 {
																								_ = rf.upto(3)
																				...
																				if postCreateEra {
																					...
																					case postCreateAssignN == 5:
																						...
																						postCreateUseExAgain = true
																					default:
																						// e15229 (n=3): bare, no afterAsg.
																						// e15731 (n=6): F50 + afterAsg.
																						// e15851 (n=7): bare + afterAsg.
																						// e15938+ (n>=8): F50 + afterAsg
																						// (bare next Function; e16061).
																						switch {
																						case postCreateAssignN == 3:
																							afterAsg = false
																						case postCreateAssignN == 6:
																							_ = rf.flipcoin(50)
																							postCreateNoAssignOnce = true
																							afterAsg = true
																						case postCreateAssignN == 7:
																							// bare + afterAsg
																							afterAsg = true
																						default:
																							if postCreateAssignStarArmed {
																								// e16096: ** (2× F50 F10);
																								// e16244: **** (4× F50 F10).
																								postCreateAssignStarArmed = false
																								postCreateAssignStarN++
																								levels := 2 * postCreateAssignStarN
																								...
																								afterAsg = false
																								postCreateUseExAgain = true
																							} else if postCreateAssignThenConst {
																								// e16496–98: F50 then bare
																								// Constant (not afterAsg).
																								postCreateAssignThenConst = false
																								_ = rf.flipcoin(50)
																								afterAsg = false
																			...
																			if vv >= 90 {
																				...
																				// Skip only when ConstSmallOnce armed (e18322 free Constant after
																				// late useEx). Sticky Hex16 from Assign must not suppress ptr-cmp.
																				if postCreatePostArrayDone && ptrCmpLhsDone {
																					if postCreateConstSmallOnce {
																						// free Constant residual takes over; drop ptr-cmp flag
																						ptrCmpLhsDone = false
																					} else {
																						ptrCmpLhsDone = false
																						// bare null; Variable RHS residual U100 scope sole
																						// Consume sticky Assign Constant arms so they do not
																						// fire residual after bare null.
																						postCreateConstBareOnce = false
																				...
																				if postCreateEra {
																					if postCreateConstF80 {
																						...
																						// e15769–83: first late Constant
																						// residual is VS Global create;
																						// e15811 bare; e15901+ F50 hex.
																						if postCreateAssignN >= 6 {
																							postCreateLateConstN++
																							if !postCreateLateCommaF80 {
																								// e16498: bare Constant after
																								// AssignThenConst; else burnConst.
																								constBare := postCreateConstBareOnce
																								if constBare {
																									postCreateConstBareOnce = false
																								} else if postCreateConstCreateArrayOnce {
																									// e18363–90: Constant after freePP CreateArray
																									// residual — SelectDeref F80 create + itemize.
																									postCreateConstCreateArrayOnce = false
																									if rf.flipcoin(80) {
																										_ = rf.flipcoin(20)
																										_ = rf.flipcoin(20)
																										...
																										_ = rf.upto(10)
																										_ = rf.upto(35)
																										// e18373–76: F20=0 → U2 (×2)
																										for ai := 0; ai < 2; ai++ {
																											if !rf.flipcoin(20) {
																												_ = rf.upto(2)
																											}
																										}
																										// e18377–88: itemize U1 U7 U10 F80 ladder
																										for ri := 0; ri < 12; ri++ {
																											_ = rf.upto(1)
																											_ = rf.upto(7)
																											_ = rf.upto(10)
																									...
																									postCreateUseExAgain = false
																									postCreateForceStdfunc = false
																									// e18391: next free Constant F50 + hex×8;
																									// e18393: next Variable tries=3 (VarOnly).
																									postCreateConstOnlyOnce = true
																									...
																									// e18322 (noVarOnly): next free Function.
																									// e17545 (default): re-arm VarOnly below.
																									noVar := postCreateConstSmallNoVarOnly
																									postCreateConstSmallOnce = false
																									postCreateConstSmallNoVarOnly = false
																								...
																								} else if postCreateLateFreeUseExN >= 3 {
																									// e18842: late free invent Constant after F0 nest
																									// F50=0 → hex multiphase (n=1 hex×4; n≥2 hex×16).
																									postCreateLateFreeConstN++
																									hexN := 4
																									if postCreateLateFreeConstN >= 2 {
																										hexN = 16 // e18907 depth gap 16
																									}
																									if rf.flipcoin(50) {
																										if rf.flipcoin(50) {
																											_ = rf.upto(3)
																								...
																								// e18842+: late free invent Constant (after F0 nest)
																								// next Variable is free (no VarOnly tries).
																								if !didAsgLhs && !postCreatePostArrayVar && !constBare {
																									if postCreateLateFreeUseExN < 3 {
																							...
																							} else if postCreateLateConstCreate {
																								postCreateLateConstCreate = false
																								_ = rf.upto(100) // Global
																								for qi := 0; qi < 3; qi++ {
																									_ = rf.flipcoin(50)
																									_ = rf.flipcoin(10)
																							...
																							} else if postCreateLateConstN == 3 {
																								// e15901–02: F50 hex×2 then
																								// next Constant tries=2.
																								if rf.flipcoin(50) {
																									if rf.flipcoin(50) {
																										_ = rf.upto(3)
																									} else {
																								...
																								// e15903: Constant-only tries=2
																								postCreateConstOnlyOnce = true
																							} else if postCreateLateConstN >= 4 {
																								// e15903–27: F50 hex×16 + F80
																								// SelectDeref + Statement residual.
																								if rf.flipcoin(50) {
																									if rf.flipcoin(50) {
																										_ = rf.upto(3)
																									} else {
																			...
																			if vv >= 70 {
																				...
																				// e16417+: post-CreateArray Variable multiphase
																				if postCreatePostArrayVar {
																					postCreatePostArrayVarN++
																				}
																				...
																				// e15965–70: after Comma Lhs residual,
																				// Variable multiphase U4 U2 U4 U2 F75
																				// (must_use-shaped) without scope U100.
																				if postCreateMustUseResidual {
																					postCreateMustUseResidual = false
																					_ = rf.upto(4)
																					_ = rf.upto(2)
																					_ = rf.upto(4)
																					_ = rf.upto(2)
																					_ = rf.flipcoin(75)
																					// e15971–78: SelectDeref F80 ladder
																					if rf.flipcoin(80) {
																						_ = rf.upto(7)
																					}
																					if rf.flipcoin(80) {
																					...
																					// e15978–23: long VS multiphase residual
																					// PL tries → PP → Global U2 F0 →
																					// PL U6 U7 → Global U2 F0 → PL U5 U7
																					// U9 U4 → PL create F5 U4 U20… For residual.
																					_ = rf.uptoWithFilter(100, func(x uint32) bool {
																						return x < 35
																					})
																					_ = rf.upto(100) // PP 83
																					_ = rf.upto(100) // PL 44
																					_ = rf.upto(100) // Global 9
																					_ = rf.upto(2)
																					_ = rf.flipcoin(0)
																					_ = rf.upto(100) // PL 45
																					_ = rf.upto(6)
																					...
																					_ = rf.flipcoin(5)
																						...
																						_ = rf.upto(4)
																						_ = rf.upto(100) // PP
																					// e16023+: next Function useExisting
																					// CREATE head F50 F20 U14.
																					postCreateUseExAgain = true
																					continue
																				}
																				sp := rf.upto(100)
																				...
																				if sp < 35 {
																					...
																					if plStackN <= 5 && depthFilterClosed && depthFilterExpr {
																						...
																						// GlobalList has grown → n=129.
																						// e15877+: late Global small U3
																						// + VS reselect U100.
																						// Accept-only: next U120 (e15388
																						// Function), not F50 trail.
																						if postCreateEra {
																							if postCreateAssignN >= 7 {
																								// e16088+: after late Comma F80
																								// era Global multiphase.
																								if !postCreateLateCommaF80 {
																									postCreateLateGlobalN++
																									if postCreateLateGlobalN == 1 {
																										// e16088: U9 F0 reselect
																										// U100 U8 + Func useEx.
																										_ = rf.upto(9)
																										_ = rf.flipcoin(0)
																										_ = rf.upto(100)
																										_ = rf.upto(8)
																										postCreateUseExAgain = true
																									} else if postCreatePostArrayDone {
																										// e16732 first: U2 F0 reselect
																										// create residual; e16866+:
																										// CreateArray residual.
																										postCreatePostArrayGlobalN++
																										if postCreatePostArrayGlobalN == 1 {
																											// e16732–42: U2 F0 reselect
																											// Global → PP U2 U14 F50
																											// F10 F20 F50; F50=0 hex×2.
																											_ = rf.upto(2)
																											_ = rf.flipcoin(0)
																											_ = rf.upto(100) // Global
																											_ = rf.upto(100) // PP
																											...
																											postCreateForceStdfunc = true
																										} else if postCreatePostArrayGlobalN == 2 {
																											// e16866–82: F50 F10×3 + F20×2
																											// F50 F20×2 U2 CreateArray
																											// U99 U10 U1 U2.
																											for qi := 0; qi < 3; qi++ {
																												_ = rf.flipcoin(50)
																												_ = rf.flipcoin(10)
																											...
																											_ = rf.upto(2)
																											postCreateVarOnlyOnce = true
																										} else if postCreatePostArrayGlobalN == 3 {
																											// e16936–41: Global ok_vars U39
																											// F80 F20 F20 U6 residual.
																											_ = rf.upto(39)
																											if rf.flipcoin(80) {
																												_ = rf.flipcoin(20)
																												_ = rf.flipcoin(20)
																											...
																											postCreateVarOnlyOnce = true
																										} else if postCreatePostArrayGlobalN == 4 {
																											// e16943–51: Global U3 reselect
																											// U100 U2 F0 → PP U2 F50 U16.
																											_ = rf.upto(3)
																											_ = rf.upto(100)
																											_ = rf.upto(2)
																											_ = rf.flipcoin(0)
																											...
																											if postCreateLateFreeGlobalN == 1 {
																												...
																												// e17547–602: U138 + nested create residual
																												// (PL F5 type ladder ×2 then free Function).
																												burnLateGlobalCreateLadder := func() {
																													_ = rf.upto(4)
																													_ = rf.upto(58)
																													_ = rf.upto(57)
																													// U56 only on first ladder (e17554); 2nd has U57 then F50
																													_ = rf.upto(56)
																													if !rf.flipcoin(50) {
																														_ = rf.upto(60)
																														_ = rf.upto(60)
																												...
																												_ = rf.upto(100) // PL
																												_ = rf.flipcoin(5)
																												burnLateGlobalCreateLadder()
																												// FactPointTo F0 → VS reselect + qfer create
																												_ = rf.flipcoin(0)
																												_ = rf.upto(100)
																												_ = rf.upto(100)
																												_ = rf.upto(6)
																												...
																												_ = rf.flipcoin(50)
																												_ = rf.flipcoin(50)
																												// e17581+: second type ladder (U3 stack + create)
																												_ = rf.upto(3)
																												_ = rf.upto(4)
																												_ = rf.upto(100)
																												_ = rf.upto(58)
																												...
																												// e17592: F50=0 → U4 (not U10-only path)
																												if !rf.flipcoin(50) {
																													_ = rf.upto(4)
																												} else {
																													_ = rf.upto(4)
																												...
																												_ = rf.upto(100)
																												_ = rf.upto(100)
																												// e17602: next Function useExisting F50 (not stdfunc F5).
																												// useEx=0 → F0 + nested Function/Constant residual
																												// (not VS U100 path).
																												postCreateForceStdfunc = false
																												postCreateUseExAgain = true
																												postCreateUseExF0NestOnce = true
																											} else if postCreateLateFreeGlobalN == 3 {
																												// e17648–: U9 F0 reselect Global U8 U2 → PP U6
																												// + qfer F50 F10 F20 + field Constant catalog
																												_ = rf.upto(9)
																												_ = rf.flipcoin(0)
																												_ = rf.upto(100) // Global
																												_ = rf.upto(8)
																												...
																												// e17658–: field catalog; hex widths from UP depth gaps
																												burnConst(8)
																												burnConst(8)
																												burnConst(8)
																												// e17667: F50=0 → hex×16 + U181 bitfield
																												if !rf.flipcoin(50) {
																													burnHex(16)
																													_ = rf.upto(181)
																												} else if rf.flipcoin(50) {
																												...
																												burnConst(8)
																												// e17672–76: F50=0 hex×8, F50=0 hex×4, F50 F50 U3
																												if !rf.flipcoin(50) {
																													burnHex(8)
																												} else if rf.flipcoin(50) {
																													_ = rf.upto(3)
																												...
																												// e17677–84: F50=0 hex×16,×8,×8,×16,×8 then F50 F50 U3
																												for _, hn := range []int{16, 8, 8, 16, 8} {
																													if !rf.flipcoin(50) {
																														burnHex(hn)
																													} else if rf.flipcoin(50) {
																														_ = rf.upto(3)
																												...
																												burnConst(8)
																												burnConst(8)
																												// e17688+: F50=0 hex×4 + F50 small U20; then free Expression
																												if !rf.flipcoin(50) {
																													burnHex(4)
																												} else if rf.flipcoin(50) {
																													_ = rf.upto(3)
																												...
																												// e18449: next Expression Variable-only.
																												_ = rf.upto(2)
																												if rf.flipcoin(80) {
																													_ = rf.upto(5)
																												}
																												...
																												// later free Global
																												// e18845: ok_vars U150 (was U140 e17436-era).
																												// e18846: SelectDeref F80=0 → VS reselect
																												// NewValue create residual (e18847–).
																												_ = rf.upto(150)
																												if !rf.flipcoin(80) {
																													sp := rf.upto(100)
																													if sp >= 95 {
																														// NewValue F10=0 → PP U6 U14 create
																														if !rf.flipcoin(10) {
																															_ = rf.upto(6)
																															// e18850: U14 tries=1 (1 untraced reject)
																															_ = rf.next31()
																															_ = rf.upto(14)
																															_ = rf.flipcoin(20)
																															_ = rf.flipcoin(50)
																															// e18852 F50=0 → hex×8 (depth 23927→23936)
																															// then SelectDeref F80 ladder
																															for h := 0; h < 8; h++ {
																																_ = rf.next31()
																															}
																															// e18853–60: F80 U6 until F80=0 → VS PP
																															// U6 U3 U1 → reselect U100 → free Expression.
																															for ri := 0; ri < 20; ri++ {
																																if !rf.flipcoin(80) {
																																	sp2 := rf.upto(100)
																																	if sp2 >= 65 && sp2 < 95 {
																																		// e18856–59: PP U6 U3 U1
																																		_ = rf.upto(6)
																																		_ = rf.upto(3)
																																		_ = rf.upto(1)
																																		// e18860: VS reselect then free invent
																																		_ = rf.upto(100)
																																	} else if sp2 < 35 {
																																		_ = rf.upto(9)
																																	} else if sp2 < 65 {
																															...
																															// e18861+: next Function useEx F50; miss → F0
																															postCreateForceStdfunc = false
																															postCreateUseExAgain = true
																										...
																										if rsp >= 95 {
																											...
																											postCreateVarOnlyOnce = true
																										} else if rsp >= 35 && rsp < 65 {
																											// e16178–80: PL U1 U5
																											_ = rf.upto(1)
																											_ = rf.upto(5)
																										} else if rsp < 35 {
																											// Global reselect sole
																										} else {
																											// PP sole
																										}
																									}
																								} else {
																									// e15877: U3 + VS reselect.
																									// e15883 (latePPN=3): U39.
																									// e15895 (latePPN=4): U38.
																									switch {
																									case postCreateLatePPN < 3:
																										_ = rf.upto(3)
																										_ = rf.upto(100)
																										postCreateVarOnlyOnce = true
																									case postCreateLatePPN == 3:
																										...
																										postCreateVarOnlyOnce = true
																									default:
																										// e15895 U38; e15897 Variable tries=1
																										_ = rf.upto(38)
																										postCreateVarOnlyOnce = true
																									}
																								}
																					...
																					if postCreatePostArrayVar && postCreatePostArrayVarN >= 3 {
																						// e16427–44: PL U1 U5 itemize
																						// U9 U9 U3 F0 → Global F0 →
																						// NewValue create residual.
																						_ = rf.upto(1)
																						_ = rf.upto(5)
																						_ = rf.upto(9)
																						...
																						_ = rf.flipcoin(0)
																						_ = rf.upto(100) // NewValue
																						// e16437–44: NewValue F10=0 PL
																						// U1 U14 F10 F20 burnConst.
																						if !rf.flipcoin(10) {
																							_ = rf.upto(1)
																							_ = rf.upto(14)
																							_ = rf.flipcoin(10)
																					...
																					if postCreateEra {
																						...
																						case postCreatePLN == 3:
																							...
																							postCreateConstOnlyOnce = true
																						case postCreatePLN == 4:
																							// e15740: stack U6 + U11
																							_ = rf.upto(6)
																							_ = rf.upto(11)
																						case postCreatePLN == 5:
																							// e15800: stack U6 + create
																							// F50 F10×3 + F10 F20 F20
																							_ = rf.upto(6)
																							for qi := 0; qi < 3; qi++ {
																								_ = rf.flipcoin(50)
																								_ = rf.flipcoin(10)
																						...
																						default:
																							if !postCreateLateCommaF80 {
																								// e16456+: late PL multiphase.
																								postCreateLatePLN++
																								if postCreateLatePLN == 1 {
																									// e16456–58: U1 U5; next VarOnly
																									_ = rf.upto(1)
																									_ = rf.upto(5)
																									postCreateVarOnlyOnce = true
																								} else if postCreateLatePLN == 2 {
																									// e16460–62: U1 U5 + F80 ladder
																									_ = rf.upto(1)
																									_ = rf.upto(5)
																									// e16463–95: F80 U7 F80=0
																									// → Global F0 + SelectDeref
																									// itemize residual.
																									if rf.flipcoin(80) {
																										_ = rf.upto(7)
																									}
																									if !rf.flipcoin(80) {
																										_ = rf.upto(100) // Global
																										_ = rf.flipcoin(0)
																										// e16468–81: F80 U5 F80
																										// U4 itemize U9 U9 U3 F0
																										// F80 U4 F80 U3 F0 F80=0
																										// → PP U1 U2 itemize…
																										if rf.flipcoin(80) {
																											_ = rf.upto(5)
																										}
																										...
																										if !rf.flipcoin(80) {
																											...
																											if !rf.flipcoin(80) {
																												...
																												_ = rf.upto(4)
																												_ = rf.upto(100) // Global
																												// e16496: next Expression
																												// no_const once (Assign tries=1);
																												// then Constant-only after Assign F50.
																												postCreateNoConstOnce = true
																												postCreateAssignThenConst = true
																											}
																										}
																									}
																								} else if postCreateLatePLN == 3 {
																									// e16500–25: late PL create
																									// U1 U14 F20 burnConst
																									// CreateArray U99 U10×2
																									// U4 U3 U3 F80=0 Global…
																									_ = rf.upto(1)
																									_ = rf.upto(14)
																									_ = rf.flipcoin(20)
																									if rf.flipcoin(50) {
																									...
																									if !rf.flipcoin(80) {
																										...
																										if !rf.flipcoin(80) {
																											...
																											_ = rf.flipcoin(0)
																											// e16519–25: F80 F20×2
																											// U5 U4 U4 U100 Global-only
																											// (tries=1 reject non-Global).
																											if rf.flipcoin(80) {
																												_ = rf.flipcoin(20)
																												_ = rf.flipcoin(20)
																												_ = rf.upto(5)
																									...
																									// e16970+: F80 CreateArray residual.
																										...
																										// e17385: first free PL after Assign Lhs create —
																										...
																										if postCreateLateFreePLN == 1 {
																										...
																										} else if postCreateLateFreePLN <= 3 {
																											// e17515, e17539: U4 sole
																											_ = rf.upto(4)
																											if postCreateLateFreePLN == 2 {
																												// e17516: next Expression Constant-only
																												// (tries=1 reject Function → Constant).
																												// F50 + hex×8 only (UP depth +8);
																												// clear sticky Hex16/asgLhs from
																												// prior Assign (ptr-cmp bare skipped).
																												postCreateConstOnlyOnce = true
																												postCreateConstHex16Once = false
																												postCreateAsgLhsF80Once = false
																												postCreateConstHex8Once = true
																											} else if postCreateLateFreePLN == 3 {
																												// e17541: Constant-only tries=2;
																												// residual F50 small (not hex8/asgLhs)
																												postCreateConstOnlyOnce = true
																												postCreateConstHex16Once = false
																												postCreateAsgLhsF80Once = false
																											...
																											_ = rf.flipcoin(50)
																											postCreateFuncOnlyOnce = true
																										} else if postCreateLateFreePLN == 7 {
																											// e18395–405: U4 U2 F0 → NV F10 U4 U14 NonVoid create;
																											// e18406: next Expression Variable-only (tries=9).
																											_ = rf.upto(4)
																											_ = rf.upto(2)
																											_ = rf.flipcoin(0)
																											_ = rf.upto(100) // NewValue
																											...
																											postCreateVarOnlyOnce = true
																										} else if postCreateLateFreePLN == 8 {
																											// e18408: U4 sole then next Variable
																											_ = rf.upto(4)
																											postCreateVarOnlyOnce = true
																										} else if postCreateLateFreePLN == 9 {
																											// e18440: U4 U5 residual
																											_ = rf.upto(4)
																											_ = rf.upto(5)
																											postCreateVarOnlyOnce = true
																										} else if postCreateLateFreePLN == 10 {
																											// e18443: U4 sole then Variable tries=5
																											_ = rf.upto(4)
																											postCreateVarOnlyOnce = true
																										} else if postCreateLateFreePLN == 11 {
																											// e18452: U4 U5 U4 reselect U100 residual
																											_ = rf.upto(4)
																											_ = rf.upto(5)
																											_ = rf.upto(4)
																											_ = rf.upto(100)
																											// e18455+: Function-shaped residual after reselect
																											_ = rf.flipcoin(5)
																											_ = rf.upto(4)
																											_ = rf.upto(23)
																											_ = rf.upto(3)
																											...
																											_ = rf.upto(4)
																											_ = rf.upto(4)
																											// e18473: VS PP-only U100 tries=1 then Assign
																											_ = rf.uptoWithFilter(100, func(x uint32) bool {
																												return x < 65 || x >= 95 // PP only
																											})
																											// e18474: next Assign + Lhs SelectDeref F80 ladder
																											postCreateForceAssignOnce = true
																											postCreateAsgSelDerefOnce = true
																										} else {
																										...
																										postCreateLatePLConstDone = true
																										postCreateConstOnlyOnce = true
																									} else if postCreatePostArrayGlobalN >= 2 {
																					...
																					// e15735: PP sole + F80 SelectDeref.
																					if postCreateEra {
																						if postCreateAssignN >= 6 {
																							// e15735: first late PP sole+F80;
																							// e15746+/e15881: later PP bare;
																							// e15886: F50 U32 residual.
																							// e16207+: after late Comma F80
																							// era, PP bare (no F50 U32).
																							postCreateLatePPN++
																							// Counts only Variable residual PP
																							// (not PL/Global trailing U100):
																							// 1 e15735 F80; 2 e15746 bare;
																							// 3 e15881 bare+VarOnly;
																							// 4 e15886 F50 U32 SelectDeref.
																							if postCreatePostArrayVar {
																								// e16417–25: post-CreateArray
																								// Variable PP multiphase.
																								if postCreatePostArrayVarN == 1 {
																									// e16418–23: U1 U2 F0 →
																									// Global U3; next VarOnly.
																									_ = rf.upto(1)
																									_ = rf.upto(2)
																									_ = rf.flipcoin(0)
																									_ = rf.upto(100) // Global
																									_ = rf.upto(3)
																									postCreateVarOnlyOnce = true
																								} else if postCreatePostArrayVarN == 2 {
																									// e16425: PP bare; next VarOnly
																									postCreateVarOnlyOnce = true
																								}
																								// n≥3: bare (PL path handles n=3)
																							} else if !postCreateLateCommaF80 {
																								// e16207+: bare until post-array
																								// done; e16450 first free bare;
																								// e16541+: U2 residual.
																								if postCreatePostArrayDone {
																									...
																									if postCreateLateFreePPN >= 2 {
																										...
																										switch {
																										case postCreateLateFreePPN >= 15:
																											// e18412: PP U4 U2 F0 reselect multiphase;
																											// e18420: next Expression Variable-only (tries=2).
																											_ = rf.upto(4)
																											_ = rf.upto(2)
																											_ = rf.flipcoin(0)
																											sp := rf.upto(100)
																										...
																										case postCreateLateFreePPN == 14:
																											// e18346: PP U4 + **** qfer F50 F10×4 + NewArray
																											// CreateArray sizes; next Expression Constant
																											// continues CreateArray residual invent.
																											_ = rf.upto(4)
																											for qi := 0; qi < 4; qi++ {
																												_ = rf.flipcoin(50)
																												_ = rf.flipcoin(10)
																											...
																											_ = rf.upto(10)
																											_ = rf.upto(4)
																											// CreateArray alts F20×3 + U9 then free Expression
																											for hi := 0; hi < 3; hi++ {
																												_ = rf.flipcoin(20)
																											}
																											_ = rf.upto(9)
																											// e18363: next Constant tries=4 + CreateArray residual
																											postCreateConstOnlyOnce = true
																											postCreateConstCreateArrayOnce = true
																											postCreateAsgLhsF80Once = false
																											postCreateConstHex16Once = false
																											postCreateConstHex8Once = false
																											postCreateConstSmallOnce = false
																										...
																										case postCreateLateFreePPN == 11:
																											// e17530: U4 F50 U16 F80 F80 F20 F20 U4
																											_ = rf.upto(4)
																											if !rf.flipcoin(50) {
																												_ = rf.upto(16)
																											} else if rf.flipcoin(50) {
																											...
																											_ = rf.flipcoin(20)
																											_ = rf.upto(4)
																										case postCreateLateFreePPN == 10:
																											// e17520: F0 → VS U100 PP U4 U2 F80 F80 U100 U5
																											// e17528: next Expression Variable-only (tries=7)
																											_ = rf.flipcoin(0)
																											sp := rf.upto(100)
																											if sp >= 65 && sp < 95 {
																												_ = rf.upto(4)
																											...
																											postCreateVarOnlyOnce = true
																										case postCreateLateFreePPN == 9:
																											// e17512: PP ok_vars U4 sole → next Expression
																											_ = rf.upto(4)
																										default:
																											_ = rf.upto(2)
																											switch {
																							...
																							} else if postCreateLatePPF80 {
																								postCreateLatePPF80 = false
																								_ = rf.flipcoin(80)
																							} else if postCreateLatePPN >= 4 {
																								// e15886–93: F50 U32 + SelectDeref
																								// F80 F20 F20 F50 U32 residual.
																								_ = rf.flipcoin(50)
																								_ = rf.upto(32)
																								if rf.flipcoin(80) {
																									_ = rf.flipcoin(20)
																								...
																								// e15894: next Variable tries=9
																								postCreateVarOnlyOnce = true
																							} else if postCreateLatePPN >= 3 {
																								// e15881+: next Variable tries=1
																								postCreateVarOnlyOnce = true
																							}
																						} else {
																							_ = rf.upto(5)
																					...
																					if postCreatePostArrayDone {
																						...
																						// e18423+: late free NewValue F10=0 → PL U4 U14 create
																						// + SelectDeref residual (not U2 CreateArray).
																						if !rf.flipcoin(10) {
																							if postCreateLateFreePPN >= 15 {
																								_ = rf.upto(4)
																								_ = rf.upto(14)
																								_ = rf.flipcoin(10)
																								_ = rf.flipcoin(20)
																								// e18427: F50=0 → RandomHexDigits(16) then SelectDeref
																								if !rf.flipcoin(50) {
																									for h := 0; h < 16; h++ {
																										_ = rf.next31()
																									}
																								...
																								// e18428–35: F80=1 U2 F0; F80=1 F0; F80=1 F20 F20 U6
																								if rf.flipcoin(80) {
																									_ = rf.upto(2)
																									_ = rf.flipcoin(0)
																								}
																			...
																			// e15097+: after Lhs SelectDeref residual
																			// ends, Function useExisting F50=1 →
																			// CREATE (make_random_signature head
																			// F20 U14 then nested Expression body).
																			// e15721+: post-CREATE second Lhs end is
																			// useExisting F50 F0 (not CREATE-head F50).
																			if lhsResidualDone {
																				lhsResidualDone = false
																				if postCreateUseExHitN >= 4 {
																					// fall through to useExisting F50 F0
																				} else if rf.flipcoin(50) {
																			...
																			if postCreateUseExAgain {
																				...
																				if useEx {
																					...
																					if postCreateLateFreeAsgN >= 1 {
																						// Count late free-asg useEx hits for multiphase.
																						postCreateLateFreeUseExN++
																						// e18810+: after field catalog Assign residual,
																						// useEx hit → CREATE head F20 U14 (not VS).
																						// Keep useExAgain for nested param Function;
																						// e18814 useEx=0 (max-funcs) → ExprVar residual.
																						if postCreateLateFreeUseExN >= 3 {
																							_ = rf.flipcoin(20)
																							_ = rf.upto(14)
																							postCreateLateFreeCreateMissVar = true
																							// keep postCreateUseExAgain for nested U120
																							continue
																						}
																						sp := rf.upto(100)
																						if sp < 35 {
																							// e18312–20: Global reselect U100
																							// + PL U6 F80=0 → PL U4 create
																							// F20 F20 U9 → next Constant.
																							_ = rf.upto(100)
																							_ = rf.upto(6)
																							if !rf.flipcoin(80) {
																								_ = rf.upto(100) // PL
																							...
																							postCreateConstOnlyOnce = true
																							postCreateConstSmallOnce = true
																							postCreateConstSmallNoVarOnly = true
																							postCreateAsgLhsF80Once = false
																							postCreateConstHex16Once = false
																							postCreateConstHex8Once = false
																							ptrCmpLhsDone = false
																							...
																							// e18303: PL U4 U3 F80;
																							// e18306: next Expression Assign-only.
																							_ = rf.upto(4)
																							_ = rf.upto(3)
																							_ = rf.flipcoin(80)
																							postCreateForceAssignOnce = true
																					...
																					if postCreateUseExHitN >= 4 {
																						// e15460 / e16024 CREATE head:
																						// F20 + U14 (may tries=2 NonVoid).
																						_ = rf.flipcoin(20)
																						if postCreateUseExHitN >= 5 {
																							// e16026: NonVoid tries
																							_ = rf.uptoWithFilter(14, func(x uint32) bool {
																								return x == 9 || x == 11 || x == 12
																							})
																							// e16028+: must_use residual era
																							postCreateUseExHitN = 100
																						} else {
																							// e15461: plain U14
																							_ = rf.upto(14)
																						}
																					...
																					sp := rf.upto(100)
																					if postCreatePostArrayDone && postCreateLateFreeGlobalN >= 3 && sp >= 65 && sp < 95 {
																						// e17695–731: late useEx PP U6 SelectDeref
																						// itemize F80 ladder → VS reselect + create.
																						_ = rf.upto(6)
																						if rf.flipcoin(80) {
																							_ = rf.upto(3)
																						}
																						...
																						if !rf.flipcoin(80) {
																							...
																							// e17706: VS U100 tries=3 (3 untraced rejects
																							// then accept; norm ignores tries) then
																							// U100 U6 U100 U100 F5 U4 type residual
																							for ti := 0; ti < 3; ti++ {
																								_ = rf.next31()
																							}
																							_ = rf.upto(100)
																							...
																							_ = rf.flipcoin(5)
																							_ = rf.upto(4)
																							// e17713–31: type/init residual then
																							// free Comma U120=118
																							_ = rf.upto(20)
																							_ = rf.upto(3)
																							_ = rf.upto(20)
																					...
																					if postCreatePostArrayDone && postCreateUseExHitN == 1 && sp >= 65 && sp < 95 {
																						// e16559–83: PP U2 F50 F10 F20
																						// + field Constants → CreateArray.
																						_ = rf.upto(2)
																						_ = rf.flipcoin(50)
																						_ = rf.flipcoin(10)
																						_ = rf.flipcoin(20)
																						...
																						// e16564–82: field catalog from UP
																						// F50 F50 U3; F50 F50 U3; F50 F50
																						// U20; F50 F50 U3; U181; F50 F50
																						// U20; F50×3 hex; U99 U10 U10.
																						burnConst(8)
																						burnConst(8)
																						burnConst(8)
																						burnConst(8)
																						_ = rf.upto(181)
																						burnConst(8)
																						// e16580–82: F50=0 hex×8,×4,×8
																						// (UP depth +9,+5,+9).
																						for _, hn := range []int{8, 4, 8} {
																							if !rf.flipcoin(50) {
																								burnHex(hn)
																							} else if rf.flipcoin(50) {
																								_ = rf.upto(3)
																						...
																						_ = rf.upto(10)
																						_ = rf.upto(10)
																						// e16586–640: post-CreateArray
																						// field Constant catalog (UP).
																						// U20; U20; hex; U3; U20; U20;
																						// hex×2 U35; U20; hex×3 U181;
																						// hex×4 U3; U3; U20; hex U181;
																						// more fields until residual end.
																						// e16586–640: field catalog with
																						// precise hex widths from UP depth.
																						// c=burnConst; hN=F50=0→hex×N.
																						hexOrConst := func(hexN int) {
																							if !rf.flipcoin(50) {
																								burnHex(hexN)
																							} else if rf.flipcoin(50) {
																								_ = rf.upto(3)
																						...
																						hexOrConst(8)
																						_ = rf.upto(35)
																						// U20; hex8 hex8 hex16 → U181
																						burnConst(8)
																						hexOrConst(8)
																						hexOrConst(8)
																						hexOrConst(16)
																						_ = rf.upto(181)
																						// hex16 hex8 hex4 hex8; U3 U3 U20;
																						// hex U181; more fields
																						hexOrConst(16)
																						hexOrConst(8)
																						hexOrConst(4)
																						hexOrConst(8)
																						...
																						hexOrConst(16)
																						_ = rf.upto(181)
																						// e16628–44: hex16 hex8; U20 U3;
																						// hex16; U20 U3; hex16 → U181
																						hexOrConst(16)
																						hexOrConst(8)
																						burnConst(8)
																						burnConst(8)
																						...
																						hexOrConst(16)
																						_ = rf.upto(181)
																						// e16645–82: hex16 hex8 hex4 U3;
																						// hex16 U3; hex8 U3; U181;
																						// U20 U3×3 U20 U3 U20; hex16 U181;
																						// more until residual ends.
																						hexOrConst(16)
																						hexOrConst(8)
																						hexOrConst(4)
																						burnConst(8)
																						...
																						hexOrConst(16)
																						_ = rf.upto(181)
																						// e16683–709: hex16 U20; hex4 hex8
																						// U10 U7; U20 hex8 U20; hex16 U3;
																						// more U20× → U120.
																						hexOrConst(16)
																						burnConst(8)
																						hexOrConst(4)
																						...
																						_ = rf.upto(10)
																						_ = rf.upto(7)
																						// e16691–709: U20; hex8 U20;
																						// hex16 U3; hex16 U20; hex4 U20
																						// → Function stdfunc.
																						burnConst(8)
																						hexOrConst(8)
																						burnConst(8)
																						...
																						burnConst(8)
																						postCreateForceStdfunc = true
																						// e16753+: Assign is F50 afterAsg
																						// (not **** WRITE residual).
																						postCreateAssignStarArmed = false
																						// e16756+: next Functions are
																						// stdfunc (not useEx again).
																						postCreateUseExAgain = false
																						continue
																					}
																					if sp < 35 {
																						...
																						if postCreateLateFreeAsgN >= 1 {
																							...
																							// e18306: next Expression Assign-only
																							// (tries=1 reject Function → Assign).
																							_ = rf.upto(4)
																							_ = rf.upto(3)
																							_ = rf.flipcoin(80)
																							postCreateForceAssignOnce = true
																				...
																				// e18814+: after late free CREATE, useEx=0
																				// (max-funcs fail) → ExpressionVariable
																				// multiphase residual (not stdfunc F5).
																				if postCreateLateFreeCreateMissVar {
																					postCreateLateFreeCreateMissVar = false
																					postCreateUseExAgain = false
																					// e18815–825 UP residual (ExprVar do-loop):
																					// PP U6=2 U3 F80=0;
																					// PP U6=0 → U100 tries=2 (2 rejects) miss;
																					// PP U6=1 U5 → U100 accept.
																					needTries2 := false
																					for ri := 0; ri < 8; ri++ {
																						var sp uint32
																						if needTries2 {
																							// e18821: U100 tries=2 — upstream
																							// filter rejects 2 draws then accepts
																							// (entropy-aligned; norm ignores tries).
																							_ = rf.next31()
																							_ = rf.next31()
																							_ = rf.upto(100)
																							needTries2 = false
																						...
																						sp = rf.upto(100)
																						if sp >= 65 && sp < 95 {
																							// ParentParam → SelectParentLocal stack
																							u6 := rf.upto(6)
																							switch {
																							case u6 >= 2:
																								// e18816–18: choose U3 F80=0 → retry
																								_ = rf.upto(3)
																								if !rf.flipcoin(80) {
																									continue
																								}
																								_ = rf.upto(5)
																							case u6 == 0:
																								// e18820–21: stack[0] miss → next
																								// U100 with tries=2 then reselect.
																								needTries2 = true
																								continue
																							default:
																								// u6==1: e18823–25 U5 + final U100
																								_ = rf.upto(5)
																								_ = rf.upto(100)
																								ri = 100
																							}
																					...
																					// e18826+: after ExprVar residual, next
																					// Function is useEx F50; miss → F0 free.
																					postCreateUseExAgain = true
																					postCreateLateFreeUseExF0Once = true
																					continue
																				}
																				...
																				// e17604: after late Global create residual,
																				// useEx=0 → F0 + nested Function binary +
																				// Constant + Function binary (not VS U100).
																				if postCreateUseExF0NestOnce {
																					postCreateUseExF0NestOnce = false
																					postCreateUseExAgain = false
																					_ = rf.flipcoin(0)
																					// nested Function binary
																					_ = rf.upto(120)
																					_ = rf.flipcoin(5)
																					_ = rf.flipcoin(10)
																					_ = rf.upto(18)
																					...
																					_ = rf.flipcoin(50)
																					_ = rf.upto(4)
																					// next free Expression (Variable e17623)
																					continue
																				}
																				// e15287–92 / e15359+ / e15464+:
																				// useExisting=0 → VS residual.
																				// e16028+: after CREATE head (sentinel
																				// hitN=100), must_use U3 U8 F75 +
																				// SelectDeref residual.
																				if postCreateUseExHitN >= 100 {
																					_ = rf.upto(3)
																					_ = rf.upto(8)
																					_ = rf.flipcoin(75)
																					// e16032–42: F80 itemize residual
																					if rf.flipcoin(80) {
																						_ = rf.upto(5)
																						_ = rf.upto(9)
																						_ = rf.upto(9)
																					...
																					// e16043: VS U100 then outer U120
																					// Variable sole; Function is stdfunc.
																					_ = rf.upto(100)
																					afterAsg = true
																					postCreateUseExAgain = false
																					postCreateUseExHitN = 0
																				...
																				if sp0 < 35 {
																					if postCreateUseExHitN >= 4 {
																						...
																						postCreateNoFuncOnce = true
																					} else if postCreateAssignStarN >= 2 {
																						// e16255–318: after **** Assign,
																						// useEx=0 Global CreateArray:
																						// F20×4 F80 F20×2 U99 U10×3 U72
																						// + long F20 alt residual.
																						for hi := 0; hi < 4; hi++ {
																							_ = rf.flipcoin(20)
																						}
																						if rf.flipcoin(80) {
																						...
																						_ = rf.upto(10)
																						_ = rf.upto(72)
																						// e16268–318: ~51 F20 alts
																						// (CreateArray init alternatives).
																						for hi := 0; hi < 51; hi++ {
																							_ = rf.flipcoin(20)
																						}
																						_ = rf.upto(6)
																						_ = rf.upto(8)
																						// e16321+: itemize U3 F0 F80
																						// ladder U6 U8 until F80=0 → VS.
																						for {
																							_ = rf.upto(3)
																							_ = rf.flipcoin(0)
																							if !rf.flipcoin(80) {
																								_ = rf.upto(100)
																								// e16330+: after first F80=0
																								// VS, continue itemize with
																								// U1 U6 U8 U3 F0 F80 ladder.
																								_ = rf.upto(1)
																								for {
																									_ = rf.upto(6)
																									_ = rf.upto(8)
																									...
																									_ = rf.flipcoin(0)
																									if !rf.flipcoin(80) {
																										// e16415–16: F80=0 → Global
																										_ = rf.upto(100)
																										break
																									}
																								}
																								// e16417+: next Expression
																								// Variable multiphase.
																								postCreatePostArrayVar = true
																								postCreateVarOnlyOnce = true
																								postCreateUseExAgain = false
																								break
																							}
																						...
																						// e16095+: Global sole (sp0 only);
																						// arm ** / **** WRITE Assign.
																						postCreateAssignStarArmed = true
																					}
																				} else if sp0 < 65 {
																					...
																					// PP: e153xx bare U5; e16103+ late
																					// era PP empty → create residual
																					// U1 F20×4 U4 F80=0 → Global
																					// CreateArray U99 U10 U5 F20×2
																					// U10 F80 F20×2 U4.
																					// e16143+: next Function is stdfunc
																					// (clear useExAgain).
																					if !postCreateLateCommaF80 {
																						_ = rf.upto(1)
																						for hi := 0; hi < 4; hi++ {
																			...
																			if rf.flipcoin(5) {
																				...
																				// e16238–44: after first ** Assign,
																				// late ptr-cmp arms **** WRITE.
																				_ = rf.flipcoin(50)
																				_ = rf.flipcoin(50)
																				_ = rf.flipcoin(50)
																				_ = rf.upto(4)
																				_ = rf.upto(24)
																				// e16238–44 **** only before post-array
																				// Variable multiphase ends.
																				if !postCreateLateCommaF80 && postCreateAssignStarN >= 1 && !postCreatePostArrayDone {
																					postCreateAssignStarArmed = true
																				}
																				if depthFilterClosed {
																					ptrCmpNoFunc = true
		...
		// e9847–48: after e9838 PL U5F0 reselect, Statement Lhs SelectDeref
		// F80 U9 accept (GO live pool under-counts as U8 → extra F80 desync).
		if ctx != nil && ctx.state != nil && ctx.state.postAggPostCD3ArrayOp2StmtLhsU9 {
			ctx.state.postAggPostCD3ArrayOp2StmtLhsU9 = false
			if !r.flipcoin(80) {
				break // VS select
			}
			_ = r.upto(9) // e9848
