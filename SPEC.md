...
**Explicitly still open (not closed by grill):** seed5 full event stream (living first_div ~6317); seeds 7–21 events; seed2/4 **source** match; stripping legacy residual/`silenceTrace` debt already in tree; full flag parity (§4 phase C). Work continues under the closed rules above.
...
| Seeds 5,7–21 event | **FAIL** — seed5 first_div **~6317** (post-ArrayOp free Expression PL: matched stack U2 + eFlexible higher-indirection choose U2; UP next U7 vs GO F80 Lhs — invent residual pack stripped); seed7+ open |
