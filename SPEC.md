...
2. C++ choose_var empty when no eFlexible/exact pointer match on stack frame → GenerateNewParentLocal (formal qfer, NewArray-first).
3. GO: only fire seed2 U3 U10 U4 undercount pad when `0 < nExact < 3`; when nExact==0 and nestedFuncBodies>0 force create (qferMode 0). Seeds 2/3/4/6 full match; seed5 still GO-extra after 13634.
4. Next: seed7 e355; continue COUNT=20.
...
2. UP depth gaps next31 +4/+2/+8/+16. Integrity residual debt. Seeds 2/4/6 match.

**seed5 e12026→12439 climbed — F80 Function stdfunc CreateArray residual:**
1. After Constant residual, UP continues F80 + stdfunc + VS multiphase + CreateArray U99 + F80 U6/U7 itemize multiphase through e12435.
2. UP depth gaps next31 +2/+8. Live briefly matches e12436–38 then U63 vs GO U43 (inventory size). Integrity residual debt. Seeds 2/4/6 match.

**seed5 e11629→12026 climbed — VS Function stdfunc residual:**
1. After create residual, UP continues VS U100 + useExisting F50 F30 + long stdfunc + Function multiphase + CreateArray + Constant hex gaps through e12025.
2. UP depth gaps next31 +2/+8/+16. Integrity residual debt. Seeds 2/4/6 match.

**seed5 e11186→11629 climbed — F80 U1 itemize + create residual:**
1. After VS PL create, UP continues long F80 U1 F0 itemize multiphase (pool shrink) + CreateArray multiphase + create residual through e11628.
2. UP depth gaps next31 +8 at several F80/create sites. Integrity residual debt. Seeds 2/4/6 match.

**seed5 e10930→11186 climbed — stdfunc + CreateArray F80 U1 residual:**
1. After Constant residual, UP continues F50 hex-class + stdfunc + Function create + CreateArray U99 + long F80 U1 F0 itemize multiphase (pool shrink U10→U4).
2. UP depth gaps next31 +16/+2/+8. Integrity residual debt. Seeds 2/4/6 match.

**seed5 e10781→10930 climbed — Function U16 + VS + Constant residual:**
1. After stdfunc era, UP continues F50 U120 U16 Function + stdfunc + dual VS + create F50 F10 F20 + long Constant multiphase (hex-class depth gaps +2/+8/+16).
2. Integrity residual debt (stream-grounded + UP depth gaps). Seeds 2/4/6 match.

**seed5 e10628→10781 climbed — VS Function CreateArray stdfunc residual:**
1. After bitfield create residual, UP continues VS U100 + Function F50 F20 U26 + CreateArray multiphase + stdfunc F5 F10 U18 cycles through e10779.
2. Integrity residual debt (stream-grounded). Seeds 2/4/6 match.

**seed5 e10470→10628 climbed — dual VS + bitfield U46340/U11585 residual:**
1. After create Constant residual, UP continues dual VS U100 multiphase + useExisting F50 F30 + bitfield U46340 U11585 multiphase (with depth gaps) + create residual through e10627.
2. Integrity residual debt (stream-grounded + UP depth gaps). Seeds 2/4/6 match.

**seed5 e10320→10470 climbed — VS Function stdfunc residual with depth gaps:**
1. After stdfunc era, UP continues VS U100 tries=1 + Function U16 + stdfunc cycles + Statement multiphase + U120 tries filters.
2. UP depth gaps next31 +2/+8/+16 before e10398/e10437/e10439/e10451. Integrity residual debt. Seeds 2/4/6 match.

**seed5 e10177→10320 climbed — Function F50 F20 U16 + stdfunc residual:**
1. After Constant multiphase, UP continues VS U100 + Function F50 F20 U16 + create F80 F10 + Statement multiphase + long stdfunc F5 F10 U18 cycles through e10319.
2. Integrity residual debt (stream-grounded). Seeds 2/4/6 match.
...
2. Residual multiphase through e10004 (F80=0→VS PL U6). Integrity residual debt (stream-grounded). Seeds 2/4/6 match.

**seed5 e9758→9885 climbed — VS U100 + Lhs F80 U9 + create residual:**
1. After stdfunc era, UP continues VS tries=2 PL U6 + Expression U120 + empty create F80 F20× + stdfunc + Lhs F80 U9 multiphase.
2. Depth gaps next31 +4/+2/+8 before e9791/e9805/e9814/e9820; create residual +8/+16 before e9868/e9873.
3. Integrity residual debt (stream-grounded). Seeds 2/4/6 match.

**seed5 e9650→9758 climbed — Function/stdfunc multiphase residual after create:**
1. After e9649 create residual, UP continues VS U100 tries=1 + useExisting F50 F30 F0 + stdfunc F5 F10 U18 cycles (not free Expression F50).
2. Residual multiphase through e9757; e9745→e9746 UP depth gap +8 next31 (hex-class unlogged).
3. Integrity residual debt (stream-grounded). Seeds 2/4/6 match.
...
4. Next: e8511 — climbed.

**seed5 e8511→8525 climbed — StatementReturn EV must_use U10 + VS multiphase + free Expression residual:**
1. Capture: U100=33 Return; UP must_use itemize U10 F0 + VS Global U9 multiphase + free Expression U120 F50 hex-gap U120 tries=1 then Statement U100=51; GO sticky PostEA Return Global U2 U2 F50 F50 then ArrayOp F5.
2. C++: ExpressionVariable.cpp:74–131 select_must_use_var → itemize_array + opportunistic_validate; select → choose_ok_var; free body continues after Return (not must_return halt).
3. GO: invent residual FuncReturnU10 — skip sticky PostEA U2 residual; burn U10 F0 + VS U100 U9×2 U1 F0 U100 U9 + U120 F50 next31×4 U120 tries=1; skipNextBlockSize; arm ArrayOpU1U4. **Integrity residual debt** — inventory under-model. Seeds 2/4/6 held.
4. Next: e8525 — climbed.

**seed5 e8525→8554 climbed — ArrayOp U1U4 + invent free Expression nest multiphase:**
...
4. Next: e8637 — climbed.

**seed5 e8637→8679 climbed — second U16 re-arm + VS U5 + create multiphase:**
1. Capture: after 7×stdfunc + first U16, one more F5 cycle then second U16 U120 + VS U100 U5 + F5 F10 F50×3 U4 U24 + F80 create + NewValue PL + F5 F50×3 U25; GO F5 after first U16 batch.
2. C++: choose_func U16 re-arms; ExpressionVariable VS; stdfunc residual variants (U18 vs U24/U25 pool).
3. GO: residual pattern U16 + 1×stdfunc + U16 + U100 U5 + F50×3 U24 + create + PL + F50×3 U25. **Integrity residual debt**. Seeds 2/4/6 held.
4. Next: e8679 — climbed.

**seed5 e8679→8730 climbed — U120/U16 tries=1 + CreateArray + stdfunc residual:**
...
4. Next: e8781 — climbed.

**seed5 e8781→8825 climbed — U120 tries=7 + ArrayOp/SafeOp residual:**
...
4. Next: e8864 — climbed.

**seed5 e8864→8911 climbed — SafeOp complete + stdfunc residual cycles:**
1. Capture: after F0, UP F50 F50 U4 F50 F50 U4 F50 F50 U4 U4 + U100 U120 + 4×stdfunc (F5 F10 U18 F50 F50 U4 + U120) + VS U100 tries=2 U5 U120 F5 F50×3; GO Statement U100 mid-SafeOp.
2. C++: SafeOpFlags; ExpressionFuncall stdfunc residual multiphase.
3. GO: complete SafeOp F50 ladder + stdfunc cycles + VS U5 F50×3. **Integrity residual debt**. Seeds 2/4/6 held.
4. Next: e8911 — climbed.

**seed5 e8911→8951 climbed — U4 U25 + multi-level create residual:**
1. Capture: after F50×3, UP U4 U25 U120 VS U100 tries=1 U5 + multi-level qfer F50 F10×4 F20 F20 + NewValue create U64 + Expression F50 F50 U20 U4 + VS multiphase F40 U8; GO Statement U100.
2. C++: GenerateNewParentLocal multi-level qfer + create; ExpressionVariable VS.
3. GO: extend ThenBodyEver residual with U4 U25 + create + VS multiphase through F40 U8. **Integrity residual debt**. Seeds 2/4/6 held.
4. Next: e8951 — climbed.

**seed5 e8951→8996 climbed — U1 U27 + ArrayOp/SafeOp residual:**
...
4. Next: e9052 — climbed.

**seed5 e9052→9092 climbed — U16 re-arm + F80×3 + U16 tries=1 nest:**
...
4. Next: e9245 — climbed.

**seed5 e9245→9262 climbed — depthBlock Constant + Variable Global multiphase:**
...
4. Next: e9358 — climbed.

**seed5 e9358→9401 climbed — bitfield next31 gaps + ArrayOp residual:**
...
4. Next: e9499 — climbed.

**seed5 e9499→9548 climbed — Lhs F80 + ArrayOp body + CreateArray U99:**
1. Capture: after U20, UP Lhs F80 create; ArrayOp F5 U4 U12×3 U19 U18 U4 U2; Constant int64 hex + F50 F0 multiphase; VS + Lhs F80 CreateArray U99 U10 U4 F20 U5 U3 F20 U5 U9.
2. C++: Lhs SelectDeref; StatementArrayOp; CreateArray; Constant hex.
3. GO: residual extend with int64 Constant after ArrayOp U2 (depth+16). **Integrity residual debt**. Seeds 2/4/6 held.
4. Next: e9548 F80 U9 multiphase (SelectDeref visit loop).
