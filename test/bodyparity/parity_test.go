...
//
// Generation contract: both sides implement csmith.Generator
// (Generate(ctx, opts) → program text). Parent owns context/timeouts.
// Go: csmith.Library (in-process). Upstream: exec of golden binary via CLIArgs.
// Fuzz mutates a compact blob decoded by OptionsFromFuzzBlob — whole Options
// surface, not seed alone.
...
// skipError is a soft reject (conflict / hang / upstream crash) — not a body mismatch.
// runGenerator maps these to testing.TB.Skip.
...
// upstreamBinary runs golden csmith via exec. Implements csmith.Generator so
// bodyparity compares two Generators with the same call shape; parent owns ctx.
type upstreamBinary struct {
	path string
}

func (u upstreamBinary) Generate(ctx context.Context, opts csmith.Options) (string, error) {
	args := opts.CLIArgs()
	...
	if ctx.Err() == context.DeadlineExceeded {
		...
		return "", skipf("upstream csmith hang/timeout args=%v (%s)", args, u.path)
	}
	...
	if err != nil {
		...
		if isUpstreamProcessCrash(err) || isUpstreamCrash(err.Error()) {
			return "", skipf("upstream crash %v: %s", args, err.Error())
		}
		msg := combined
		if msg == "" {
			msg = err.Error()
		...
		if isUpstreamConflict(msg) {
			return "", skipf("upstream rejects config %v: %s", args, firstLine(msg))
		}
		...
		if isUpstreamCrash(msg) {
			return "", skipf("upstream crash %v: %s", args, firstLine(msg))
		}
		return "", fmt.Errorf("upstream csmith %v (%s): %s", args, u.path, firstLine(msg))
	}
	...
	if isUpstreamConflict(combined) && !strings.Contains(stdout.String(), "int main") {
		return "", skipf("upstream rejects config %v: %s", args, firstLine(combined))
	}
	return stdout.String(), nil
}

// runGenerator calls g.Generate with a parent-owned timeout on tb.Context().
// Soft rejects (skipError) become tb.Skip; hard errors Fatal.
func runGenerator(tb testing.TB, g csmith.Generator, opts csmith.Options, timeout time.Duration) string {
	tb.Helper()
	...
	if ctx.Err() == context.DeadlineExceeded {
		tb.Fatalf("%T hang/timeout after %s args=%v", g, timeout, opts.CLIArgs())
	}
	if strings.Contains(err.Error(), "conflict") || strings.Contains(err.Error(), "generation error") {
		tb.Skipf("generate skipped: %v (args=%v)", err, opts.CLIArgs())
	}
	tb.Fatalf("%T args=%v: %v", g, opts.CLIArgs(), err)
	return ""
}
...
// goLibrary is csmith.Library with Validate soft-skip for invalid drop-in configs.
// Implements csmith.Generator (same interface as upstreamBinary).
type goLibrary struct {
	csmith.Library
}

var (
	_ csmith.Generator = goLibrary{}
	_ csmith.Generator = upstreamBinary{}
)

func (g goLibrary) Generate(ctx context.Context, opts csmith.Options) (string, error) {
	if err := opts.Validate(); err != nil {
		return "", skipf("go rejects config: %v", err)
	}
	// Header Options: line should list the same argv we pass upstream.
	opts.Argv = opts.CLIArgs()
	return g.Library.Generate(ctx, opts)
}
...
func assertOptsBodyParity(tb testing.TB, opts csmith.Options) {
	...
	// Two Generators, same interface; parent owns timeouts on tb.Context().
	up := upstreamBinary{path: upstreamCsmith(tb)}
	goSide := goLibrary{}
	upTo := upstreamGenTimeout
	if opts.DFSExhaustive {
		upTo = upstreamDFSTimeout
	}
	upOut := runGenerator(tb, up, opts, upTo)
	goOut := runGenerator(tb, goSide, opts, goGenTimeout)
